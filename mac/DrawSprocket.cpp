...
//		gDSpContextAttributes[i].frequency 				= 75<<16;
	...
	gDSpContextAttributes[current_context].contextOptions		= 0 | kDSpContextOption_DontSyncVBL;	// no page flipping and no VBL sync needed
	...
	for(i = 0; i < N_SUPPORTED_VIDRES; i++) {
		if(i != current_context) {
			...
			gDSpContextAttributes[i].displayHeight		= Video_res_list[i].height;
			gDSpContextAttributes[i].pageCount			= 1;									// only the front buffer is needed
			gDSpContextAttributes[i].contextOptions	= 0 | kDSpContextOption_DontSyncVBL;	// no page flipping and no VBL sync needed
		}
	}
	// reserve our menu context
	err = DSpContext_Reserve( gDSpContext[current_context], &gDSpContextAttributes[current_context] );
	...
	// create a new window in our context (required for multi-monitor handling (for single monitor it would be better to just use the context
	// Note: OpenGL is expecting a window so it can enumerate the devices it is on, if you us a CGrafPtr instead it MUST be on the main device always
		SetRect(&rectWin, 0, 0, 100, 100);
		*ppWin = (CGrafPtr)NewCWindow (NULL, &rectWin, "\pDescent 3", 0, plainDBox, (WindowPtr)-1, 0, 0);
		WindowReset((GrafPtr)*ppWin, gDSpContextAttributes[current_context].displayWidth, gDSpContextAttributes[current_context].displayHeight);
...
void SwitchDSpContex(int newContext)
{
	...
	if(gDSpContext[current_context] && gDSpContext[newContext]) {
		...
		WindowReset((GrafPtr)macApp.hwnd, gDSpContextAttributes[newContext].displayWidth, gDSpContextAttributes[newContext].displayHeight);
...
void ResumeDSpContext()
{
	...
	WindowReset((GrafPtr)macApp.hwnd, gDSpContextAttributes[current_context].displayWidth, gDSpContextAttributes[current_context].displayHeight);
}
