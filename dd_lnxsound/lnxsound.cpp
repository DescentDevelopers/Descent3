...
bool lnxsound::LockSound(int sound_uid)
{
	return false;
}

bool lnxsound::UnlockSound(int sound_uid)
{
	return false;
}
...
char lnxsound::GetSoundQuality(void)
{
	return m_sound_quality;
}

bool lnxsound::SetSoundMixer(char mixer_type)
{
	return true;
}
	
char lnxsound::GetSoundMixer(void)
{
	return SOUND_MIXER_SOFTWARE_16;
}
...
inline int lnxsound::MakeUniqueId(int sound_slot)
{
...
int lnxsound::PlaySound3d(play_information *play_info, int sound_index, pos_state *cur_pos, float adjusted_volume, bool f_looped, float reverb) //, unsigned short frequency
{
	short sound_slot;
	int ds_flags = 0;
	float volume;
...
void lnxsound::AdjustSound(int sound_uid, pos_state *cur_pos, float adjusted_volume, float reverb)
{
	...
	{
		volume *= (1.0 - ((dist - Sounds[sound_cache[current_slot].m_sound_index].min_distance) / (Sounds[sound_cache[current_slot].m_sound_index].max_distance - Sounds[sound_cache[current_slot].m_sound_index].min_distance)));
	}
...
bool lnxsound::SetGlobalReverbProperties(float volume,float damping,float decay)
{
	return false;
}
...
int lnxsound_buffer::GetNumBufferBytes(void)
{
