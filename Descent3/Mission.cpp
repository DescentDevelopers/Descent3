		...
		//	then parse out remainder.
			...
			// first check if number of level is greater than num_levels
				...
				if (curlvlnum == 0) {
					...
					if (!(lvls[curlvlnum-1].filename = mem_strdup(operand))) goto fatal_error; 
				}
			}
...
void DoMissionMovie(char *movie)
{
	...
	{		
		...
		if(Current_mission.filename && !(!stricmp(Current_mission.filename,"d3.mn3")||!stricmp(Current_mission.filename,"d3_2.mn3")))
		{
...
//@@	if (!fp) {
//@@		DoMessageBox("Error", "Unable to find mission script.", MSGBOX_OK, UICOL_WINDOW_TITLE, UICOL_TEXT_NORMAL);
