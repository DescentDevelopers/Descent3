...
int GetTextEffectWithTabStopsOnScreen(int screen)
{
	...
	while(node!=-1){
		if(TCEffects[node].type==EFX_TEXT_STATIC ||
			TCEffects[node].type==EFX_TEXT_FADE ||
...
void RenderTextStatic(tceffect *tce,float frametime,int xoff,int yoff,bool ok_to_render)
{
	...
	{
		grtext_Printf(tce->pos_x+glitch_dx+xoff,curry+glitch_dy,buff); lcount++;
		curry += height;
		nextline = textaux_CopyTextLine(nextline,buff);
	}
	grtext_Printf(tce->pos_x+glitch_dx+xoff,curry+glitch_dy,buff);	lcount++;
...
void RenderTextType(tceffect *tce,float frametime,int xoff,int yoff,bool ok_to_render)
{
	...
	int lcount = 0;
	while((nextline)&&((curry+height)<=tce->pos_y+tce->h) ){
		grtext_Printf(tce->pos_x+xoff+glitch_dx,curry+yoff+glitch_dy,buff);	lcount++;		
		curry += height;
		nextline = textaux_CopyTextLine(nextline,buff);
	}
	grtext_Printf(tce->pos_x+xoff+glitch_dx,curry+yoff+glitch_dy,buff);	lcount++;
...
void RenderTextFade(tceffect *tce,float frametime,int xoff,int yoff,bool ok_to_render)
{
	...
	int lcount = 0;
	while((nextline)&&((curry+height)<tce->pos_y+tce->h) ){
		grtext_Printf(tce->pos_x+glitch_dx+xoff,curry+glitch_dy+yoff,buff);lcount++;
		curry += height;
		nextline = textaux_CopyTextLine(nextline,buff);
	}
	grtext_Printf(tce->pos_x+glitch_dx+xoff,curry+glitch_dy+yoff,buff);lcount++;
...
void RenderBmpScanline(tceffect *tce,float frametime,int xoff,int yoff,bool ok_to_render)
{
	...
	for (yy=stage=0;yy<h;yy++){
		...
		switch (stage) {
			...
			case 1: stage = 3; break;
			case 2: stage = 1; break;
			case 3: stage = 0; break;
		}
	}
...
void RenderBmpStretch(tceffect *tce,float frametime,int xoff,int yoff,bool ok_to_render)
{
	...
	for(dx=0,realx=startx;dx<w_count;dx++,realx+=w){
		rend_DrawScaledBitmap(realx+xoff+glitch_dx,realy+yoff+glitch_dy,realx+w+xoff+glitch_dx,realy+h+yoff+glitch_dy,tce->bmpinfo.bitmaps[index],0,0,1,1);
		index++;
	}
	rend_SetFiltering(1);
	tce->age+=frametime;
...
void RenderButton(tceffect *tce,float frametime,int xoff,int yoff,bool ok_to_render)
{
	...
	if(tce->buttoninfo.flash_time>0 && !glow )
		time_to_flash = false;
	if(tce->buttoninfo.flash_state && (tce->buttoninfo.button_type==BUTT_UPARROW||tce->buttoninfo.button_type==BUTT_DOWNARROW))
	{
	...
	if(tce->buttoninfo.flash_handle==-1 && time_to_flash)
		time_to_flash = false;
	if(tce->has_focus){
		BltBmpToScreen(tce->pos_x+glitch_dx,tce->pos_y+glitch_dy,(time_to_flash)?&tce->buttoninfo.flashfocus_chunk:&tce->buttoninfo.chunkfocus_bmp);
	}else{
		BltBmpToScreen(tce->pos_x+glitch_dx,tce->pos_y+glitch_dy,(time_to_flash)?&tce->buttoninfo.flash_chunk:&tce->buttoninfo.chunk_bmp);
	}
}
