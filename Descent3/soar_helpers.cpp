...
{
	if(n < 0 || n > Highest_object_index || Objects[n].type == OBJ_NONE) return false;
...
{
	if(n < 0 || n > Highest_object_index) return false;
...
bool DSSetControllerInfo(int n, game_controls *controls)
{
	...
	if(n < 0 || n > Highest_object_index || Objects[n].type == OBJ_NONE || Objects[n].control_type != CT_SOAR) return false;
...
bool DSGetFirstRoomObject(int n, int *first)
{
	...
	{
		if(n < 0 || n > Highest_room_index || Rooms[n].used == 0) return false;
	...
	else
	{
		...
		cell = CELLNUM(n);
		if(cell < 0 && cell >= TERRAIN_WIDTH * TERRAIN_DEPTH) return false;
...
{
	if(n < 0 || n > Highest_room_index || Rooms[n].used == 0) return false;
	if(p < 0 || p >= Rooms[n].num_portals) return false;
...
{
	if(n < 0 || n > Highest_object_index || Objects[n].type != OBJ_DOOR) return false;
...
void DSSoarInit()
{
	...
	SoarInit((void *)Terrain_seg, sizeof(Terrain_seg[0]), (void *)Rooms, sizeof(Rooms[0]), (void *)Objects, sizeof(Objects[0]), (void *)dsfunc);
