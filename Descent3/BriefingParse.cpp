...
#define PARSE_INT(i)		do {p = ParseInt(p,&(i)); if (!p) goto done_parsing;} while (0)
#define PARSE_FLOAT(i)		do {p = ParseFloat(p,&(i)); if (!p) goto done_parsing;} while (0)
#define PARSE_COMMA()		do {p = ParseComma(p); if (!p) goto done_parsing;} while (0)
#define PARSE_STRING(buf)	do {p = ParseString(p,buf,sizeof(buf)); if (!p) goto done_parsing;} while (0)
#define PARSE_TOKEN(buf)	do {p = ParseToken(p,buf,sizeof(buf)); if (!p) goto done_parsing;} while (0)
			...
			case K_SOUND:
				{
					...
					while(!play){
						...
						if(!stricmp(buffer,"once")){
							snddesc.caps |= TCSD_ONCE;
							snddesc.once = true;
						}
						else
						if(!stricmp(buffer,"starttime")){
			...
			case K_SCREEN:
				{
					...
					if(StartScreen)
						(*StartScreen)(current_screen,description,(layout_found)?layout_scr:NULL,mission_mask_set,mission_mask_unset);
			...
			case K_ENDTEXT:
				{
					...
					if(AddTextEffect)
						(*AddTextEffect)(&text_buffer_desc.textdesc,text_buf,text_buffer_desc.description,text_buffer_desc.text_id);
					text_buffer_desc.text_id = -1;
					text_ptr = NULL;
				}
				break;
			case K_BITMAP:
				{
					...
					while(!done){
						...
						if(!stricmp(buffer,"no_early_rend")){
							bmpdesc.flags |= TC_NOEARLYRENDER;
						}
						else
						if(!stricmp(buffer,"effect")){
							found = false;
							i=0;
							PARSE_TOKEN(buffer);
							while(!found){
								...
								if(!strnicmp(BitmapEffectstr[i],buffer,strlen(BitmapEffectstr[i]))){
									beffect = i;
									found = true;
								}
								i++;
						...
						if(!stricmp(buffer,"position")){
							PARSE_INT(x);
							PARSE_COMMA();
							PARSE_INT(y);
							bmpdesc.caps|=TCBD_XY;
							bmpdesc.x = x; bmpdesc.y = y;
						}else
						if(!stricmp(buffer,"isset")){
							int value,bit = 0x01;
							PARSE_INT(value);
			...
			case K_MOVIE:
				{
					...
					while(!done){
						...
						if(!stricmp(buffer,"fps")){
							PARSE_FLOAT(fps);
							moviedesc.fps = fps;
							moviedesc.caps|=TCMD_FPS;
						}
						else
						if(!stricmp(buffer,"looping")){
...
bool CBriefParse::ParseButtonEffect(char *p)
{
	...
	while(!show){
		...
		if(!stricmp(buffer,"id")){
			PARSE_INT(id);
		}
		else
		if(!stricmp(buffer,"parent_id")){
			PARSE_INT(p_id);
		}
		else
		if(!stricmp(buffer,"sibling_id")){
			PARSE_INT(s_id);
		}
		else
		if(!stricmp(buffer,"desc")){
	...
	switch(type){
	case osbDOWN_ARROW:				desc.button_type = BUTT_DOWNARROW;	break;
	case osbUP_ARROW:				desc.button_type = BUTT_UPARROW;	break;
	case osbNEXT_PAGE:				desc.button_type = BUTT_NEXTPAGE;	break;
	case osbPREV_PAGE:				desc.button_type = BUTT_PREVPAGE;	break;
	case osbQUIT:					desc.button_type = BUTT_QUIT;		break;
	case osbJUMP_PAGE:				desc.button_type = BUTT_JUMP; desc.jump_page = screen; break;
	default:
	...
	switch(ctype){
	case oscHOLD_DOWN:				desc.click_type = CLICKTYPE_DOWN;		break;
	case oscCLICK_DOWN:				desc.click_type = CLICKTYPE_CLICKDOWN;	break;
	case oscCLICK_UP:				desc.click_type = CLICKTYPE_CLICKUP;	break;
	default:
	...
	while(!done){
		...
		if(!stricmp(buffer,"effect")){
			...
			while(!found){
				...
				if(!strnicmp(TextEffectstr[i],buffer,strlen(TextEffectstr[i]))){
					tbd->teffect = i;
					found = true;
				}
				i++;
		...
		if(!stricmp(buffer,"font")){
			...
			while(!found){
				...
				if(!strnicmp(FontNames[i],buffer,strlen(FontNames[i]))){
					feffect = i;
					found = true;
				}
				i++;
