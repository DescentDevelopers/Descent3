...
UIObject *NewUIGameWindowCreate(int x, int y, int w, int h, int flags = UIF_PROCESS_ALL);
...
UIItem *CreateNewUITextItem(const char *newtext, uint32_t color, int font = -1);
...
UIObject *SliderCreate(UIWindow *parent, int id, int x, int y, int flags = 0);
