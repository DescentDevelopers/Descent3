...
#define TEST_MODULE(x) { if(!(x)){ mprintf((0,"IntelliVIBE unable to find: %s\n",#x)); mod_FreeModule(&IntelliVIBE_module); IntelliVIBE_module.handle = NULL; return;} }
void VIBE_Init(oeApplication *app)
{
	...
	IVIBE_DoQuaterFrame = (IntelliVIBE_DoQuaterFrame_fp)mod_GetSymbol(&IntelliVIBE_module,"IntelliVIBE_DoQuaterFrame",4);
...
void VIBE_DoQuaterFrame(bool first_frame)
{
	...
	if(first_frame)
	{
		...
		IntelliVIBE_state.force_info.force_vector.x = IntelliVIBE_state.force_info.force_vector.y = IntelliVIBE_state.force_info.force_vector.z = 0;
	...
	IntelliVIBE_state.flags &= ~(VIBEFLAG_QUATERFRAME_0|VIBEFLAG_QUATERFRAME_1|VIBEFLAG_QUATERFRAME_2|VIBEFLAG_QUATERFRAME_3);

	// set the flag for which quaterframe we are on
	switch(quater_frame_count)
	{
	case 0: IntelliVIBE_state.flags |= VIBEFLAG_QUATERFRAME_0; break;
	case 1: IntelliVIBE_state.flags |= VIBEFLAG_QUATERFRAME_1; break;
	case 2: IntelliVIBE_state.flags |= VIBEFLAG_QUATERFRAME_2; break;
	case 3: IntelliVIBE_state.flags |= VIBEFLAG_QUATERFRAME_3; break;
	}
...
void VIBE_PlayerRespawn(void)
{
	IntelliVIBE_state.flags |= VIBEFLAG_PLAYER_RESPAWN;
}

void VIBE_PlayerDeath(void)
{
	IntelliVIBE_state.flags |= VIBEFLAG_PLAYER_DEAD;
}
