...
#define GET_AND_VERIFY_OBJECT(o) {o=VerifyMSafeObject(MultiGetUshort (data,&count)); if (o==OBJECT_HANDLE_NONE) return; }
...
//			break;
		...
		case MSAFE_MISC_HUD_MESSAGE:
			...
			if (mstruct->state)
			{
				ASSERT (Objects[mstruct->objhandle & HANDLE_OBJNUM_MASK].type==OBJ_PLAYER || Objects[mstruct->objhandle & HANDLE_OBJNUM_MASK].type==OBJ_OBSERVER || Objects[mstruct->objhandle & HANDLE_OBJNUM_MASK].type==OBJ_GHOST);
			}
			MultiAddUshort (mstruct->objhandle & HANDLE_OBJNUM_MASK,data,&count);
			MultiAddInt (mstruct->color,data,&count);
			MultiAddString (mstruct->message,data,&count);
