...
{
	return (((counter&0xFF)<<16) + (slot&0xFF));
}
...
int SpewCreate(spewinfo *spew)
{
	...
	if(spew->drag<0) spew->drag = 0;
	if(spew->effect_type<0 || spew->effect_type>=NUM_FIREBALLS) return -1;
	if(spew->lifetime<0) spew->lifetime = 0;
	if(spew->longevity<0) spew->longevity = 0;
	if(spew->mass<0) spew->mass = 0;
	if(spew->size<1) spew->size = 1;
	if(spew->speed<0) spew->speed = 0;
	if(spew->time_int<=0) spew->time_int = 0.001f;
			...
			if (spew->time_until_next_blob >= 0 ){
				...
				if(spew->use_gunpoint){
					...
					if(obj){
						...
						if((obj->flags&OF_MOVED_THIS_FRAME)  || ((obj->flags & OF_ATTACHED) && ObjGet(obj->attach_ultimate_handle) && ObjGet(obj->attach_ultimate_handle)->movement_type == MT_PHYSICS && (ObjGet(obj->attach_ultimate_handle)->flags & OF_MOVED_THIS_FRAME)))
				...
				if(spew->use_gunpoint)
				{
					...
					if(obj)
					{
						...
						VisEffectCreateControlled(VIS_FIREBALL,obj,(int)spew->effect_type,obj->roomnum,&spew->gun_point,
							lifetime,&vel_vector,spew->phys_info,size,spew->drag,spew->mass,spew->real_obj);
					}
					else
					{
