...
void QuickSaveGame() {
  ...
  if (Quicksave_game_slot == -1) {
    ...
    ddio_MakePath(pathname, GetWritableBaseDirectory().string().c_str(), "savegame", filename, NULL);
...
void SaveGameDialog() {
  ...
  // setup paths.
  ddio_MakePath(savegame_dir, GetWritableBaseDirectory().string().c_str(), "savegame", NULL);
