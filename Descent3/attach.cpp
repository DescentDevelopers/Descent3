...
//				rot_angle = (vm_DeltaAngVecNorm(&start_uvec, &goal_uvec, &rot_vec)/65535.0f) * (2.0f * PI);
				rot_angle = acos(dot);
			}
			else 
			{
...
bool AttachPointPos(object *obj, char ap, bool f_compute_pos, vector *attach_pos, bool f_compute_fvec, vector *attach_fvec, bool *f_computed_uvec = NULL, vector *attach_uvec = NULL)
{
	poly_model *pm;
	vector pnt;
	vector normal;
...
bool AttachDoPosOrient(object *parent, char parent_ap, object *child, char child_ap, bool f_parent, bool f_move_obj, vector *pos, matrix *orient, bool f_dropping_off)
{
	vector goal_fvec;
	vector goal_uvec;
	vector goal_pos;
