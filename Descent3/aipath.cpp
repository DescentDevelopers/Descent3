...
//	mprintf((0, "AI: Finding an alternate path from %d to %d\n", i, j));
	...
	while(cur_node = PQPath.pop()) 
	{
		...
		for(counter = 0; counter < num_portals; counter++) 
		{
			...
			{
				next_portal = BOA_DetermineStartRoomPortal(BOA_INDEX(next_room), NULL, BOA_INDEX(cur_node->roomnum), NULL, false, false, NULL);
			}
			...
			if(BOA_cost_array[cur_node->roomnum][counter] >= 0.0f)
				new_cost = cur_node->cost + (1.0f + 0.1f*((float)ps_rand() - (float)RAND_MAX/2.0f)/((float)RAND_MAX/2.0f)) * (BOA_cost_array[cur_node->roomnum][counter] + BOA_cost_array[BOA_INDEX(next_room)][next_portal]);
			else
				continue;
...
{
	if(aip->cur_path == aip->num_paths - 1 && 
...
void AIPathComplete(object *obj)
{
	GoalPathComplete(obj);
}
...
//	mprintf((0, "%d is at Node %d\n", OBJNUM(obj), n));
	...
	if(((!f_reverse && AIPathAtEnd(aip)) || (f_reverse && AIPathAtStart(aip))) && (((goal *)(&obj->ai_info->goals[aip->goal_index]))->circle_distance == 0.0f) && obj->movement_type == MT_PHYSICS)
	{
	...
	if(!f_pass_node)
	{
		if(!f_reverse)
		{
			...
			else
			{
				...
				if(proj_len >= line_len) f_pass_node = true;
			}
		}
		else
		{
...
//		mprintf((0, "f_pass_node\n"));
		...
		if(((f_reverse) && AIPathAtStart(aip)) ||
			((!f_reverse) && AIPathAtEnd(aip)))
		{
...
//			mprintf((0, "Passed node\n"));
		...
		if(n != last_n || p != last_p) goto pass_node;
	}
}
...
inline bool AIPathAddDPathNode(ai_path_info *aip, int *slot, int *cur_node, vector *pos, int room, int handle)
{
	...
	if(*cur_node == MAX_NODES)
	{
		...
		if(!status) Int3();  //chrishack -- out of dynamic paths
...
bool AIGenerateAltBNodePath(object *obj, vector *start_pos, int *start_room, vector *end_pos, int *end_room, ai_path_info *aip, int *slot, int *cur_node, int handle)
{
	int x;
	vector *pos;
...
//	if(ROOMNUM_OUTSIDE(next_room) && ROOMNUM_OUTSIDE(*end_room) && (TERRAIN_REGION(next_room) == TERRAIN_REGION(*end_room) ||
//		TERRAIN_REGION(*end_room) == 0 || TERRAIN_REGION(next_room) == 0))
//	{
//		next_room = *end_room;
//	}

bool AIGenerateBNodePath(object *obj, vector *start_pos, int *start_room, vector *end_pos, int *end_room, ai_path_info *aip, int *slot, int *cur_node, int handle)
{
...
//	AIPathAddDPathNode(aip, slot, cur_node, pos, *start_room, handle);
	...
	while((BOA_INDEX(next_room) != BOA_GetNextRoom(next_room, *end_room)) && (next_room != BOA_NO_PATH) && f_path_exists)
	{
		int cur_room;
...
bool AIGenerateBOAPath(vector *start_pos, int *start_room, vector *end_pos, int *end_room, ai_path_info *aip, int *slot, int *cur_node, int handle)
{
...
bool AIPathAllocPath(object *obj, ai_frame *ai_info, void *goal_ptr, int *start_room, vector *start_pos, int *end_room, vector *end_pos, float rad, int flags, int handle, int ignore_obj)
{
	bool status;
	bool f_path_exists = true;
...
//	if(ai_info->awareness <= AWARE_NONE && !(ai_info->flags & AIF_PERSISTANT))  // chrishack -- scalar max_nodes in path by awareness
//		return false;
	...
	if(status)
	{
		...
		if(BOA_GetNextRoom(*start_room, *end_room) != BOA_NO_PATH)
		{
			...
			if(fate == HIT_NONE || ((fate == HIT_OBJECT || fate == HIT_SPHERE_2_POLY_OBJECT) && hit_info.hit_object[0] == ignore_obj))
			{
				f_make_alt = false;
				f_bline_ok = true;
			}
			...
			if(f_make_alt)
			{
				...
				if(f_path_exists)
				{
					bool f_bnode_ok =  BNode_allocated && 
						                BNode_verified && 
					...
					if(f_bnode_ok)
						f_path_exists = AIGenerateAltBNodePath(obj, start_pos, start_room, end_pos, end_room, aip, &slot, &cur_node, handle);
					else
			...
			else if(!f_bline_ok)
			{
				...
				if(f_bnode_ok)
					f_path_exists = AIGenerateBNodePath(obj, start_pos, start_room, end_pos, end_room, aip, &slot, &cur_node, handle);
				else
...
bool MakeTestPath(int *start_room, vector *pos)
{
	...
	status = AIPathAllocPath(NULL, &ai_info, &ai_info.goals[0], start_room, pos, &AIPath_test_end_room, &AIPath_test_end_pos, 0.0f, 0, 0);
