...
char *Anim_state_names[]={"Alert","Death","Fire Missile 1","Missile Recoil 1",
						  "Fire Missile 2","Missile Recoil 2","Melee 1","Melee Recoil 1",
						  "Melee 2","Melee Recoil 2","Idle","Quirk","Flinch","Taunt",
						  "To Standing Idle","To Flying Idle","To Rolling Idle",";To Walking Idle","To Jumping Idle",
						  "Goto standing","Goto flying","Goto rolling","Goto walking",
						  "Goto jumping"};
...
void DrawObject(object *obj)
{
	...
	if (obj->render_type == RT_POLYOBJ)
		DrawPolygonModel(&obj->pos,&obj->orient,Object_info[obj->id].render_handle,NULL,0,1.0,1.0,1.0, obj->rtype.pobj_info.subobj_flags);
	else
		Int3();
...
//			int new_index = AllocObjectID(Object_info[i].type, Object_info[i].flags);
			if(Object_info[i].anim) f_anim = true;
			if(Object_info[i].static_wb) f_weapons = true;
			if(Object_info[i].ai_info) f_ai = true;
