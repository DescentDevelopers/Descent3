		...
		if(whats_there[count]==HOTSPOT_THERE)
		{
			...
			for(y=0;y<height;y++)
			{
				for(x=0;x<width;x++)
				{
					...
					{
						if(last_y!=y) sl_count++;
						if(hsmap->hs[count].starting_y>y)
							hsmap->hs[count].starting_y=y;
						if(sl_x[y].start>x) sl_x[y].start=x;
						if(sl_x[y].end<x) sl_x[y].end=x;
						last_y=y;
					}
				}
			}
...
void CreateWindowMap(char *map,int width,int height,windowmap_t *wndmap)
{
	...
	{
		wndmap->wm[count].x=wndmap->wm[count].y=
			wndmap->wm[count].width=wndmap->wm[count].height=-1;
	...
	for(count=0;count<wndmap->num_of_windows;count++)
	{
		mprintf((0,"Window #%d: Left/Top=(%d,%d) Width=%d Height=%d\n",count,
			wndmap->wm[count].x,
			wndmap->wm[count].y,
			wndmap->wm[count].width,
			wndmap->wm[count].height));
		mprintf((0,"---L.T. (%d,%d)->(%d,%d)\n",
			wndmap->wm[count].l_start_x,
			wndmap->wm[count].t_top_y,
			wndmap->wm[count].l_end_x,
			wndmap->wm[count].t_bottom_y));
		mprintf((0,"---R.T. (%d,%d)->(%d,%d)\n",
			wndmap->wm[count].r_start_x,
			wndmap->wm[count].t_top_y,
			wndmap->wm[count].r_end_x,
			wndmap->wm[count].t_bottom_y));
		mprintf((0,"---L.B. (%d,%d)->(%d,%d)\n",
			wndmap->wm[count].l_start_x,
			wndmap->wm[count].b_top_y,
			wndmap->wm[count].l_end_x,
			wndmap->wm[count].b_bottom_y));
...
void menutga_SaveHotSpotMap(char *filename,hotspotmap_t *hsmap,windowmap_t *wndmap)
{
	...
	for(count=0;count<wndmap->num_of_windows;count++)
	{
		...
		{
			//left top
			size=((wndmap->wm[count].l_end_x)-(wndmap->wm[count].l_start_x))*((wndmap->wm[count].t_bottom_y)-(wndmap->wm[count].t_top_y));
			for(index=0;index<size;index++)
			{
				cf_WriteByte(file,wndmap->wm[count].lt[index]);
			}
			//right top
			size=((wndmap->wm[count].r_end_x)-(wndmap->wm[count].r_start_x))*((wndmap->wm[count].t_bottom_y)-(wndmap->wm[count].t_top_y));
			for(index=0;index<size;index++)
			{
		...
		{
			//left bottom
			size=((wndmap->wm[count].l_end_x)-(wndmap->wm[count].l_start_x))*((wndmap->wm[count].b_bottom_y)-(wndmap->wm[count].b_top_y));
			for(index=0;index<size;index++)
			{
				cf_WriteByte(file,wndmap->wm[count].lb[index]);
			}
			//right bottom
			size=((wndmap->wm[count].r_end_x)-(wndmap->wm[count].r_start_x))*((wndmap->wm[count].b_bottom_y)-(wndmap->wm[count].b_top_y));
			for(index=0;index<size;index++)
			{
...
void menutga_LoadHotSpotMap(int back_bmp, char *filename,hotspotmap_t *hsmap,windowmap_t *wndmap)
{
	...
	for(count=0;count<wndmap->num_of_windows;count++)
	{
		if(wndmap->wm[count].t_top_y!=-1)
		{
			...
			makecorner(wndmap->wm[count].lt_bmp,back_bmp,wndmap->wm[count].lt,wndmap->wm[count].l_start_x,wndmap->wm[count].t_top_y,
			...
			wndmap->wm[count].rt_bmp=bm_AllocBitmap(wndmap->wm[count].r_end_x-wndmap->wm[count].r_start_x,
										wndmap->wm[count].t_bottom_y-wndmap->wm[count].t_top_y,0);
			makecorner(wndmap->wm[count].rt_bmp,back_bmp,wndmap->wm[count].rt,wndmap->wm[count].r_start_x,wndmap->wm[count].t_top_y,
				wndmap->wm[count].r_end_x,wndmap->wm[count].t_bottom_y);
			if(wndmap->wm[count].lt) mem_free(wndmap->wm[count].lt);
			if(wndmap->wm[count].rt) mem_free(wndmap->wm[count].rt);
		}
		...
		if(wndmap->wm[count].b_top_y!=-1)
		{
			...
			makecorner(wndmap->wm[count].lb_bmp,back_bmp,wndmap->wm[count].lb,wndmap->wm[count].l_start_x,wndmap->wm[count].b_top_y,
			...
			wndmap->wm[count].rb_bmp=bm_AllocBitmap(wndmap->wm[count].r_end_x-wndmap->wm[count].r_start_x,
									wndmap->wm[count].b_bottom_y-wndmap->wm[count].b_top_y,0);
			makecorner(wndmap->wm[count].rb_bmp,back_bmp,wndmap->wm[count].rb,wndmap->wm[count].r_start_x,wndmap->wm[count].b_top_y,
				wndmap->wm[count].r_end_x,wndmap->wm[count].b_bottom_y);
			if(wndmap->wm[count].lb) mem_free(wndmap->wm[count].lb);
			if(wndmap->wm[count].rb) mem_free(wndmap->wm[count].rb);
		}
	}
}
...
bool menutga_ConvertTGAtoHSM(char *fpath)
{
	...
	if(bm_handle==-1) return false;
	wndmap.num_of_windows=CreateHotSpotMap(map[0],width,height,&hsmap);
	if(wndmap.num_of_windows==-1) return false;		//DAJ -1FIX
	...
	{
		if(wndmap.wm[index].lt) mem_free(wndmap.wm[index].lt);
		if(wndmap.wm[index].rt) mem_free(wndmap.wm[index].rt);
		if(wndmap.wm[index].lb) mem_free(wndmap.wm[index].lb);
		if(wndmap.wm[index].rb) mem_free(wndmap.wm[index].rb);
	}
	if(wndmap.wm) mem_free(wndmap.wm);
	return true;
}
...
//@@			if(wndmap->wm[count].t_top_y!=-1)
//@@			{
//@@				Game_viewport->fillrect(GR_RGB(128,128,128), wndmap->wm[count].l_start_x,wndmap->wm[count].t_top_y,
//@@					wndmap->wm[count].l_end_x,wndmap->wm[count].t_bottom_y);
//@@				Game_viewport->fillrect(GR_RGB(128,128,128), wndmap->wm[count].r_start_x,wndmap->wm[count].t_top_y,
//@@					wndmap->wm[count].r_end_x,wndmap->wm[count].t_bottom_y);
//@@			}
//@@			if(wndmap->wm[count].b_top_y!=-1)
//@@			{
//@@				Game_viewport->fillrect(GR_RGB(128,128,128), wndmap->wm[count].l_start_x,wndmap->wm[count].b_top_y,
//@@					wndmap->wm[count].l_end_x,wndmap->wm[count].b_bottom_y);
//@@				Game_viewport->fillrect(GR_RGB(128,128,128), wndmap->wm[count].r_start_x,wndmap->wm[count].b_top_y,
//@@					wndmap->wm[count].r_end_x,wndmap->wm[count].b_bottom_y);
//@@			}
//@@
//@@		}
...
void ExportHotSpot(char *filename,hotspotmap_t *hsmap)
{
	...
	for(i=0;i<hsmap->num_of_hotspots;i++)
	{
		...
		{
			sprintf(buffer,"----Y=%d StartX=%d EndX=%d",hsmap->hs[i].starting_y+j,hsmap->hs[i].x[j].start,hsmap->hs[i].x[j].end);
			cf_WriteString(file,buffer);
		}
	}
