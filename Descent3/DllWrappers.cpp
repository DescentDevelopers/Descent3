...
{
	*parent = ObjGetUltimateParent(child);
}
...
{
	return Database->write(label,entry);
}
...
{
	return UnattachChild(parent,parent_ap);
}
...
{
	return UnattachChildren(parent);
}
...
bool dInven_Add(Inventory *inven,int type,int id,object *parent,int aux_type,int aux_id,int flags,char *description)
{
...
bool dInven_Remove(Inventory *inven,int type,int id)
{
	return inven->Remove(type,id);
}

//uses an item in the inventory (also reduces its count by one...if there is no more, then it goes bye-bye)
bool dInven_Use(Inventory *inven,int type,int id,object *parent)
{
	return inven->Use(type,id,parent);
}

//uses an item in the inventory (given it's objhandle).
bool dInven_UseObjHandle(Inventory *inven,int objhandle,object *parent)
{
	return inven->Use(objhandle,parent);
}
...
bool dInven_CheckItem(Inventory *inven,int type,int id)
{
	return inven->CheckItem(type,id);
}
...
void dInven_Reset(Inventory *inven,bool in_game,int reset_stage)
{
	inven->Reset(in_game,reset_stage);
}
...
void dInven_NextPos(Inventory *inven,bool skip)
{
	inven->NextPos(skip);
}
...
//skip : if true then skip over non-selectable items
void dInven_PrevPos(Inventory *inven,bool skip)
{
	inven->PrevPos(skip);
}
...
{
	return inven->GetPosTypeID(type,id);
}
...
{
	inven->GotoPos(type,id);
}

//uses the currently selected item
bool dInven_UsePos(Inventory *inven,object *parent)
{
	return inven->UsePos(parent);
}
...
void dInven_ValidatePos(Inventory *inven,bool forward)
{
	inven->ValidatePos(forward);
}

//returns whether an item is selectable
bool dInven_IsSelectable(Inventory *inven)
{
	return inven->IsSelectable();
}

bool dInven_IsUsable(Inventory *inven)
{
	return inven->IsUsable();
}
...
{
	return inven->GetTypeIDCount(type,id);
}

//searches the inventory for the specified type/id, sets the pos to it
//for object handle items, pass the object handle in as the type and leave
//id as -1
bool dInven_FindPos(Inventory *inven,int type,int id)
{
	return inven->FindPos(type,id);
}
