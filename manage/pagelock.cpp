...
int mng_MakeLocker()
{
	...
	do {
		...
		if (locker) {
			strupr(locker);
			if (OutrageMessageBox (MBOX_YESNO, "The network database is already locked by %s.\nIt will probably become available if you wait a few moments.\n\nWould you like to try again?",locker) != IDYES)
				return 0;
		}
	} while (locker);
...
int mng_ReplacePagelock (char *name,mngs_Pagelock *pl)
{
	...
	while (!done)
	{
		...
		else
		{
			...
			else strcpy (InfoString,"Page was successfully replaced.");
			done=1;
		}
	}
	cfclose (infile);
	...
	while (!done)
	{
		if (mng_ReadPagelock (infile,&temp_pl))
		{
			if (!stricmp(temp_pl.name,name) && pagetype==temp_pl.pagetype)
				deleted=1;
			else mng_WritePagelock (outfile,&temp_pl);
...
int mng_GetListOfLocks (mngs_Pagelock *pl,int max,char *who)
{
	...
	while (!done)
	{
		...
		if (mng_ReadPagelock (infile,&temp_pl))
		{
			if (((who == NULL) && stricmp(temp_pl.holder,"UNLOCKED")) || ((who != NULL) && (!stricmp (temp_pl.holder,who))))
			{
				pl[num]=temp_pl;
				num++;
				if (num>=max)
...
void mng_OverrideToUnlocked (mngs_Pagelock *temp_pl)
{
	...
	int answer=OutrageMessageBox (MBOX_YESNO, "Are you sure you wish to override this page to unlocked? (choose 'NO' if you don't know what you're doing!)");

	if (answer==IDNO)
		return;

	answer=OutrageMessageBox (MBOX_YESNO, "Do you wish to actually erase this pagelock? (Again, answer NO if you are not sure!)");
