...
int mng_GetGuaranteedWeaponPage (char *name,CFILE *infile)
{
	...
	//ASSERT (ret>=0);
	if (ret < 0)
		DataError("Error loading weapon <%s>: %s, file <%s>\n",weaponpage.weapon_struct.name,Weapon_error,Weapon_error_filename);
...
void mng_AssignWeaponToWeaponPage(int n,mngs_weapon_page *weaponpage)
{
	...
	if ((weaponpointer->flags & WF_SMOKE) && weaponpointer->smoke_handle>=0 && GameTextures[weaponpointer->smoke_handle].used)
	{	
