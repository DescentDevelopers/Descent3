...
void mng_WriteShipPage (CFILE *outfile,mngs_ship_page *shippage)
{
	...
	for(i = 0; i < MAX_PLAYER_WEAPONS; i++)
	{
		...
		for(j = 0; j < MAX_WB_GUNPOINTS; j++)  cf_WriteShort(outfile, shippage->ship_struct.static_wb[i].gp_weapon_index[j]);
		...
		for(j = 0; j < MAX_WB_FIRING_MASKS; j++)  cf_WriteByte(outfile, shippage->ship_struct.static_wb[i].gp_fire_masks[j]);
		for(j = 0; j < MAX_WB_FIRING_MASKS; j++)  cf_WriteFloat(outfile, shippage->ship_struct.static_wb[i].gp_fire_wait[j]);
	...
	while (!done)
	{
		...
		switch (command)
		{
			...
			case SHIPPAGE_COMMAND_WB_INFO: 
			{
				...
				if(version < 1)
				{
					for(j = 0; j < MAX_WB_GUNPOINTS; j++)  shippage->ship_struct.static_wb[i].gp_weapon_index[j] = cf_ReadInt(infile);
					shippage->ship_struct.static_wb[i].aiming_gp_index = cf_ReadInt(infile);
				}
				else
				{
					for(j = 0; j < MAX_WB_GUNPOINTS; j++)  shippage->ship_struct.static_wb[i].gp_weapon_index[j] = cf_ReadShort(infile);
				...
				for(j = 0; j < MAX_WB_FIRING_MASKS; j++)  shippage->ship_struct.static_wb[i].gp_fire_masks[j] = cf_ReadByte(infile);
				for(j = 0; j < MAX_WB_FIRING_MASKS; j++)  shippage->ship_struct.static_wb[i].gp_fire_wait[j]  = cf_ReadFloat(infile);
...
int mng_AssignShipPageToShip(mngs_ship_page *shippage,int n,CFILE *infile)
{
	...
	for(i = 0; i < MAX_PLAYER_WEAPONS; i++)
	{
		...
		if(shippage->spew_powerup_name[i][0] != 0)
		{
			...
			if (shippointer->spew_powerup[i] > -1 && Object_info[shippointer->spew_powerup[i]].type != OBJ_POWERUP) {	//DAJ -1FIX
	...
	if (mng_ReadNewShipPage (infile,&shippage))
	{
		...
		if (i != -1) {
			...
			if (pm->n_attach < 3) {															//check for enough attach points
				DataError("Model <%s> for ship <%s> has %d attach points (must have at least 3)\n",pm->name,Ships[i].name,pm->n_attach);
			}
			FreePolymodelData(Ships[i].model_handle);		//Unload the data so it will get loaded later after the textures are in memory
		}
	}
	else
