...
void UIListBox::SetSelectChangeCallback(void (*fn)(int))
{
	selectchange_fn = fn;
}
	...
	if (btn == UILMSEBTN) {
		...
		// handle repeat scrolling (note this is if the user has scrolled once already.
			...
			if (UI_TIME() < (m_ClickTime+UI_DBLCLICK_DELAY)) {
				if (abs(m_LastMseX - m_MouseX) < UI_DBLCLICK_MSEDELTA && 
					abs(m_LastMseY - m_MouseY) < UI_DBLCLICK_MSEDELTA) {
...
void UIListBox::SetSelectedColor(ddgr_color col)
{
	m_SelectColor = col;
}

void UIListBox::SetHiliteColor(ddgr_color col)
{
	m_HiliteColor = col;
}
