...
int CScriptDictionary::GetRefCount() { return refCount & 0x7FFFFFFF; }

void CScriptDictionary::SetGCFlag() { refCount |= 0x80000000; }

bool CScriptDictionary::GetGCFlag() { return (refCount & 0x80000000) ? true : false; }
...
// different type.
void CScriptDictionary::Set(const string &key, asINT64 &value) { Set(key, &value, asTYPEID_INT64); }
...
void CScriptDictionary::Set(const string &key, double &value) { Set(key, &value, asTYPEID_DOUBLE); }
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine) {
  ...
  assert(r >= 0);
  // Use the generic interface to construct the object since we need the engine pointer, we could also have retrieved
  // the engine pointer from the active context
  ...
  r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()", asMETHOD(CScriptDictionary, AddRef),
                                      asCALL_THISCALL);
  assert(r >= 0);
  r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASE, "void f()", asMETHOD(CScriptDictionary, Release),
                                      asCALL_THISCALL);
  assert(r >= 0);

  r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, ?&in)",
  ...
  r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, int64&in)",
  ...
  r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, double&in)",
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod("dictionary", "void delete(const string &in)", asMETHOD(CScriptDictionary, Delete),
                                   asCALL_THISCALL);
  assert(r >= 0);
...
void RegisterScriptDictionary_Generic(asIScriptEngine *engine) {
  ...
  assert(r >= 0);
  r = engine->RegisterObjectMethod("dictionary", "void delete(const string &in)",
