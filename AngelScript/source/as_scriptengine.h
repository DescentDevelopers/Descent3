...
// TODO: Deprecate GetSizeOfPrimitiveType. This function is not necessary now that all primitive types have fixed
// typeIds

// TODO: DiscardModule should take an optional pointer to asIScriptModule instead of module name. If null, nothing is
// done.
...
public:
  ...
  virtual int GetGlobalPropertyByIndex(asUINT index, const char **name, int *typeId = 0, bool *isConst = 0,
                                       const char **configGroup = 0, void **pointer = 0);
...
public:
  ...
  bool GenerateNewTemplateFunction(asCObjectType *templateType, asCObjectType *templateInstanceType,
                                   asCDataType &subType, asCScriptFunction *templateFunc, asCScriptFunction **newFunc);
