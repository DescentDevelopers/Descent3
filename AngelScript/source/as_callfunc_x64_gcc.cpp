...
#define ASM_GET_REG(name, dest) __asm__ __volatile__("mov  %" name ", %0\r\n" : : "m"(dest))
...
static asQWORD X64_CallFunction(const asDWORD *pArgs, const asBYTE *pArgsType, void *func) {
  ...
  for (i = MAX_CALL_INT_REGISTERS + MAX_CALL_SSE_REGISTERS;
       pArgsType[i] != x64ENDARG && (i < X64_MAX_ARGS + MAX_CALL_SSE_REGISTERS + 3); i++) {
...
int CallSystemFunction(int id, asCContext *context, void *objectPointer) {
  ...
  for (n = 0; (n < X64_CALLSTACK_SIZE) && (used_int_regs < MAX_CALL_INT_REGISTERS); n++) {
    if (argsType[n] == x64INTARG) {
      ...
      memcpy(tempBuff + idx * CALLSTACK_MULTIPLIER, paramBuffer + n * CALLSTACK_MULTIPLIER,
             CALLSTACK_MULTIPLIER * sizeof(asDWORD));
      base_n++;
      used_int_regs++;
    }
  }
