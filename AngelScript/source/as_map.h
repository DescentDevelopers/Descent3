...
template <class KEY, class VAL> asCMap<KEY, VAL>::~asCMap() { EraseAll(); }
...
template <class KEY, class VAL> int asCMap<KEY, VAL>::GetCount() { return count; }
...
template <class KEY, class VAL> int asCMap<KEY, VAL>::CheckIntegrity(asSMapNode<KEY, VAL> *node) {
  if (node == 0) {
    if (root == 0)
      return 0;
    else if (ISRED(root))
...
template <class KEY, class VAL> bool asCMap<KEY, VAL>::MoveTo(asSMapNode<KEY, VAL> **out, const KEY &key) {
  asSMapNode<KEY, VAL> *p = root;
  while (p) {
    if (key < p->key)
      p = p->left;
...
template <class KEY, class VAL> bool asCMap<KEY, VAL>::MoveFirst(asSMapNode<KEY, VAL> **out) {
  *out = root;
  if (root == 0)
    return false;
...
template <class KEY, class VAL> bool asCMap<KEY, VAL>::MoveLast(asSMapNode<KEY, VAL> **out) {
  *out = root;
  if (root == 0)
    return false;
