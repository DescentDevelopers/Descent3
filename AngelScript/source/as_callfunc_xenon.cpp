...
extern "C" {
enum argTypes { ppcENDARG = 0, ppcINTARG, ppcFLOATARG, ppcDOUBLEARG };
...
//--------------------------------------------------------------------
{
  ...
  if (iCallConv == ICC_GENERIC_FUNC || iCallConv == ICC_GENERIC_METHOD)
  ...
  if (pObj && iCallConv != ICC_CDECL_OBJLAST && iCallConv != ICC_CDECL_OBJLAST_RETURNINMEM) {
  ...
  if (pSysFunc->takesObjByVal) {
    ...
    for (asUINT uParam = 0; uParam < pDescr->parameterTypes.GetLength(); uParam++) {
      ...
      if (pDescr->parameterTypes[uParam].IsObject() && !pDescr->parameterTypes[uParam].IsObjectHandle() &&
  ...
  // Copy the parameter types for the ppcFunc. Also copy the params for the fixed xenon args buffer /containing this and
  // return ptr/
  for (int iParam = 0; iParam < (int)pDescr->parameterTypes.GetLength(); iParam++, iArgsPtr++) {
    ...
    // If the arg is bool, then endian swap it /it would not neccessary if the AS_BIG_ENDIAN would swap the bool from
    // 0x01000000 to 0x00000010/
  ...
  if (pSysFunc->takesObjByVal) {
    ...
    if (iCallConv >= ICC_THISCALL && !pObjectPointer)
      pArgs++;

    int iSpos = 0;
    for (int iParam = 0; iParam < (int)descr->parameterTypes.GetLength(); iParam++) {
      if (pDescr->parameterTypes[iParam].IsObject() && !pDescr->parameterTypes[iParam].IsReference() &&
  ...
  if (pDescr->returnType.IsObject() && !pDescr->returnType.IsReference()) {
    if (pDescr->returnType.IsObjectHandle()) {
      pContext->regs.objectRegister = (void *)(asDWORD)dwRetQW;

      if (pSysFunc->returnAutoHandle && pContext->regs.objectRegister)
    ...
    // If the retval is bool, then endian swap it /it would not neccessary if the AS_BIG_ENDIAN would swap the bool from
    // 0x01000000 to 0x00000010/
