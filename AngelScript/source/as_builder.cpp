...
int asCBuilder::CompileFunction(const char *sectionName, const char *code, int lineOffset, asDWORD compileFlags,
  ...
  GetParsedFunctionDetails(node, scripts[0], 0, func->name, func->returnType, func->parameterTypes, func->inOutFlags,
                           func->isReadOnly, isConstructor, isDestructor);
...
asCGlobalProperty *asCBuilder::GetGlobalProperty(const char *prop, bool *isCompiled, bool *isPureConstant,
  ...
  for (n = 0; n < gvars->GetLength(); ++n) {
    if ((*gvars)[n] && (*gvars)[n]->name == prop) {
      if (isCompiled)
        *isCompiled = (*gvars)[n]->isCompiled;

      if (isPureConstant)
        *isPureConstant = (*gvars)[n]->isPureConstant;
      if (constantValue)
        *constantValue = (*gvars)[n]->constantValue;
...
int asCBuilder::ParseFunctionDeclaration(asCObjectType *objType, const char *decl, asCScriptFunction *func,
  ...
  if (isSystemFunction && (func->returnType.GetObjectType() && (func->returnType.GetObjectType()->flags & asOBJ_REF)) &&
  ...
  while (n) {
    ...
    if (isSystemFunction && (type.GetObjectType() && (type.GetObjectType()->flags & asOBJ_REF)) &&
...
int asCBuilder::RegisterGlobalVar(asCScriptNode *node, asCScriptCode *file) {
  ...
  while (n) {
    ...
    gvar->nextNode = 0;
    if (n->next &&
        (n->next->nodeType == snAssignment || n->next->nodeType == snArgList || n->next->nodeType == snInitList)) {
  ...
  for (n = 0; n < classDeclarations.GetLength(); n++) {
    ...
    if (decl->objType->derivedFrom) {
      ...
      for (asUINT p = 0; p < baseType->properties.GetLength(); p++) {
        asCObjectProperty *prop =
            AddPropertyToClass(decl, baseType->properties[p]->name, baseType->properties[p]->type);
    ...
    for (asUINT m = 0; m < decl->objType->methods.GetLength(); m++) {
      ...
      if (func->funcType != asFUNC_VIRTUAL) {
        ...
        decl->objType->methods.PushLast(
            CreateVirtualFunction(func, (int)decl->objType->virtualFunctionTable.GetLength() - 1));
        m--;
      }
    }
...
asCObjectProperty *asCBuilder::AddPropertyToClass(sClassDeclaration *decl, const asCString &name, const asCDataType &dt,
  ...
  if (propSize == 2 && (decl->objType->size & 1))
    decl->objType->size += 1;
  if (propSize > 2 && (decl->objType->size & 3))
    decl->objType->size += 4 - (decl->objType->size & 3);
...
void asCBuilder::AddDefaultConstructor(asCObjectType *objType, asCScriptCode *file) {
  ...
  module->AddScriptFunction(file->idx, funcId, objType->name.AddressOf(), returnType, parameterTypes.AddressOf(),
                            inOutFlags.AddressOf(), (asUINT)parameterTypes.GetLength(), false, objType);
  ...
  module->AddScriptFunction(file->idx, funcId, objType->name.AddressOf(), returnType, parameterTypes.AddressOf(),
                            inOutFlags.AddressOf(), (asUINT)parameterTypes.GetLength(), false);
  functions.PushLast(0);
  asCCompiler compiler(engine);
...
void asCBuilder::GetParsedFunctionDetails(asCScriptNode *node, asCScriptCode *file, asCObjectType *objType,
                                          ...
                                          bool &isConstMethod, bool &isConstructor, bool &isDestructor) {
  // Find the name
  isConstructor = false;
  isDestructor = false;
  asCScriptNode *n = 0;
...
int asCBuilder::RegisterScriptFunction(int funcID, asCScriptNode *node, asCScriptCode *file, asCObjectType *objType,
                                       bool isInterface, bool isGlobalFunction) {
  asCString name;
  ...
  GetParsedFunctionDetails(node, file, objType, name, returnType, parameterTypes, inOutFlags, isConstMethod,
  ...
  module->AddScriptFunction(file->idx, funcID, name.AddressOf(), returnType, parameterTypes.AddressOf(),
                            inOutFlags.AddressOf(), (asUINT)parameterTypes.GetLength(), isInterface, objType,
                            isConstMethod, isGlobalFunction);
  ...
  if (objType) {
    ...
    if (isConstructor) {
      ...
      module->AddScriptFunction(file->idx, factoryId, name.AddressOf(), dt, parameterTypes.AddressOf(),
                                inOutFlags.AddressOf(), (asUINT)parameterTypes.GetLength(), false);
...
int asCBuilder::RegisterImportedFunction(int importID, asCScriptNode *node, asCScriptCode *file) {
  ...
  if (funcs.GetLength()) {
    for (asUINT n = 0; n < funcs.GetLength(); ++n) {
      ...
      if (name == func->name && parameterTypes.GetLength() == func->parameterTypes.GetLength()) {
        bool match = true;
        for (asUINT p = 0; p < parameterTypes.GetLength(); ++p) {
          if (parameterTypes[p] != func->parameterTypes[p]) {
            match = false;
  ...
  // Register the function
  module->AddImportedFunction(importID, name.AddressOf(), returnType, parameterTypes.AddressOf(),
                              inOutFlags.AddressOf(), (asUINT)parameterTypes.GetLength(), moduleName);
...
void asCBuilder::GetFunctionDescriptions(const char *name, asCArray<int> &funcs) {
  ...
  for (n = 0; n < module->scriptFunctions.GetLength(); n++) {
    if (module->scriptFunctions[n]->name == name && module->scriptFunctions[n]->objectType == 0)
  ...
  for (n = 0; n < engine->scriptFunctions.GetLength(); n++) {
    if (engine->scriptFunctions[n] && engine->scriptFunctions[n]->funcType == asFUNC_SYSTEM &&
        engine->scriptFunctions[n]->objectType == 0 && engine->scriptFunctions[n]->name == name) {
...
asCDataType asCBuilder::CreateDataTypeFromNode(asCScriptNode *node, asCScriptCode *file, bool acceptHandleForScope,
  ...
  if (n->tokenType == ttIdentifier) {
    ...
    if (templateType && (templateType->flags & asOBJ_TEMPLATE) &&
        str == templateType->templateSubType.GetObjectType()->name)
    ...
    if (ot == 0) {
      ...
      if (!module || !group || group->HasModuleAccess(module->name.AddressOf())) {
        if (asOBJ_TYPEDEF == (ot->flags & asOBJ_TYPEDEF)) {
          // TODO: typedef: A typedef should be considered different from the original type (though with implicit
          // conversions between the two) Create primitive data type based on object flags
          ...
          if (ot->flags & asOBJ_TEMPLATE) {
            ...
            if (subType.GetObjectType() != ot->templateSubType.GetObjectType()) {
              ...
              if (!otInstance) {
                asCString msg;
                msg.Format(TXT_CANNOT_INSTANCIATE_TEMPLATE_s_WITH_s, ot->name.AddressOf(),
                           subType.Format().AddressOf());
                int r, c;
                file->ConvertPosToRowCol(n->tokenPos, &r, &c);
                WriteError(file->name.AddressOf(), msg.AddressOf(), r, c);
              }
...
asCDataType asCBuilder::ModifyDataTypeFromNode(const asCDataType &type, asCScriptNode *node, asCScriptCode *file,
  ...
  if (n && n->tokenType == ttAmp) {
    ...
    if (!engine->ep.allowUnsafeReferences && inOutFlags && *inOutFlags == asTM_INOUTREF) {
      ...
      if (!dt.IsObject() || dt.IsObjectHandle() || !dt.GetObjectType()->beh.addref ||
          !dt.GetObjectType()->beh.release) {
        int r, c;
...
int asCBuilder::GetEnumValueFromObjectType(asCObjectType *objType, const char *name, asCDataType &outDt,
                                           asDWORD &outValue) {
