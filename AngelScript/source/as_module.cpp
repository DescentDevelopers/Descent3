...
// interface
asIScriptEngine *asCModule::GetEngine() { return engine; }

// interface
void asCModule::SetName(const char *name) { this->name = name; }

// interface
const char *asCModule::GetName() { return name.AddressOf(); }
...
int asCModule::AddScriptSection(const char *name, const char *code, size_t codeLength, int lineOffset) {
  ...
  builder->AddCode(name, code, (int)codeLength, lineOffset, (int)engine->GetScriptSectionNameIndex(name ? name : ""),
                   engine->ep.copyScriptSections);
...
// interface
int asCModule::GetImportedFunctionCount() { return (int)bindInformations.GetLength(); }
...
// interface
int asCModule::GetFunctionCount() { return (int)globalFunctions.GetLength(); }
...
int asCModule::GetFunctionIdByDecl(const char *decl) {
  ...
  int id = -1;
  for (size_t n = 0; n < globalFunctions.GetLength(); ++n) {
    if (globalFunctions[n]->objectType == 0 && func.name == globalFunctions[n]->name &&
...
// interface
int asCModule::GetGlobalVarCount() { return (int)scriptGlobals.GetLength(); }
...
int asCModule::GetGlobalVarIndexByDecl(const char *decl) {
  ...
  int id = -1;
  for (size_t n = 0; n < scriptGlobals.GetLength(); ++n) {
    if (gvar.name == scriptGlobals[n]->name && gvar.type == scriptGlobals[n]->type) {
      id = (int)n;
      break;
    }
  }
...
// interface
int asCModule::GetObjectTypeCount() { return (int)classTypes.GetLength(); }
...
// interface
int asCModule::GetEnumCount() { return (int)enumTypes.GetLength(); }
...
// interface
int asCModule::GetTypedefCount() { return (int)typeDefs.GetLength(); }
...
// internal
int asCModule::GetNextImportedFunctionId() { return FUNC_IMPORTED | (asUINT)engine->importedFunctions.GetLength(); }

// internal
int asCModule::AddScriptFunction(int sectionIdx, int id, const char *name, const asCDataType &returnType,
                                 asCDataType *params, asETypeModifiers *inOutFlags, int paramCount, bool isInterface,
                                 asCObjectType *objType, bool isConstMethod, bool isGlobalFunction) {
  asASSERT(id >= 0);
...
// internal
int asCModule::AddImportedFunction(int id, const char *name, const asCDataType &returnType, asCDataType *params,
                                   asETypeModifiers *inOutFlags, int paramCount, const asCString &moduleName) {
  asASSERT(id >= 0);
...
bool asCModule::AreInterfacesEqual(asCObjectType *a, asCObjectType *b, asCArray<sObjectTypePair> &equals) {
  ...
  for (unsigned int n = 0; n < a->methods.GetLength(); n++) {
    ...
    if (funcA->name != funcB->name || funcA->parameterTypes.GetLength() != funcB->parameterTypes.GetLength())
      break;
