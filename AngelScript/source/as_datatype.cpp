...
bool asCDataType::IsNullHandle() const {
  if (tokenType == ttUnrecognizedToken && objectType == 0 && isObjectHandle)
    return true;
...
int asCDataType::MakeHandle(bool b, bool acceptHandleForScope) {
  if (!b) {
    ...
    if (!objectType || !((objectType->flags & asOBJ_REF) || (objectType->flags & asOBJ_TEMPLATE_SUBTYPE)) ||
        (objectType->flags & asOBJ_NOHANDLE) || ((objectType->flags & asOBJ_SCOPED) && !acceptHandleForScope))
      return -1;
...
bool asCDataType::SupportHandles() const {
  if (objectType && (objectType->flags & asOBJ_REF) && !(objectType->flags & asOBJ_NOHANDLE) && !isObjectHandle)
    return true;
...
bool asCDataType::CanBeInstanciated() const {
  if (GetSizeOnStackDWords() == 0 ||
      (IsObject() && (objectType->flags & asOBJ_REF) && // It's a ref type and
...
bool asCDataType::CanBeCopied() const {
  ...
  if (objectType->beh.construct == 0 && objectType->beh.factory == 0)
    return false;

  // It must be possible to copy the type
  if (objectType->beh.copy == 0)
    return false;
...
bool asCDataType::operator!=(const asCDataType &dt) const { return !(*this == dt); }
...
bool asCDataType::IsEqualExceptInterfaceType(const asCDataType &dt) const {
  ...
  if (objectType != dt.objectType) {
    if (!objectType || !dt.objectType)
      return false;
    if (!objectType->IsInterface() || !dt.objectType->IsInterface())
      return false;
  }
...
bool asCDataType::IsSamePrimitiveBaseType(const asCDataType &dt) const {
  if (!IsPrimitive() || !dt.IsPrimitive())
    return false;

  if (IsIntegerType() && dt.IsIntegerType())
    return true;
  if (IsUnsignedType() && dt.IsUnsignedType())
    return true;
  if (IsFloatType() && dt.IsFloatType())
    return true;
  if (IsDoubleType() && dt.IsDoubleType())
    return true;
  if (IsBooleanType() && dt.IsBooleanType())
    return true;
  if (IsFloatType() && dt.IsDoubleType())
    return true;
  if (IsDoubleType() && dt.IsFloatType())
    return true;
...
asSTypeBehaviour *asCDataType::GetBehaviour() const { return objectType ? &objectType->beh : 0; }
