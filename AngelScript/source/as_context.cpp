...
class asCDebugStats {
public:
  asCDebugStats() { memset(instrCount, 0, sizeof(instrCount)); }
...
asCContext::~asCContext() { DetachEngine(); }

int asCContext::AddRef() { return refCount.atomicInc(); }
...
asIScriptEngine *asCContext::GetEngine() { return engine; }
...
void *asCContext::GetUserData() { return userData; }
...
int asCContext::GetCallstackSize() { return (int)callStack.GetLength() / CALLSTACK_FRAME_SIZE; }

int asCContext::GetCallstackFunction(int index) {
  if (index < 0 || index >= GetCallstackSize())
    return asINVALID_ARG;
...
void asCContext::ExecuteNext() {
  ...
  for (;;) {
    ...
    case asBC_CMPd: {
      ...
      if (dbl == 0)
        *(int *)&regs.valueRegister = 0;
      else if (dbl < 0)
        *(int *)&regs.valueRegister = -1;
      else
        *(int *)&regs.valueRegister = 1;
      l_bc += 2;
    } break;
...
if (instr != asBC_JMP && instr != asBC_JMPP && (instr < asBC_JZ || instr > asBC_JNP) && instr != asBC_CALL &&
    instr != asBC_CALLBND && instr != asBC_CALLINTF && instr != asBC_RET && instr != asBC_ALLOC) {
...
asEContextState asCContext::GetState() { return status; }
...
void asCContext::ClearExceptionCallback() { exceptionCallback = false; }
