...
//       I'm pretty sure that PPC 32bit sets up a stack frame slightly different (only 24 bytes for linkage area for
//       instance)
...
// StackArgSizeInBytes is the size in bytes of the stack frame (takes into account linkage area, etc. must be multiple
// of 16)
...
asm(""
    ...
    "stdux %r1, %r1, %r4\n"   // atomically store and update the stack pointer for the new stack frame (in case of a
                              // signal/interrupt)
...
inline bool IsVariableArgument(asCDataType type) { return (type.GetTokenType() == ttQuestion) ? true : false; }
...
int CallSystemFunction(int id, asCContext *context, void *objectPointer) {
  ...
  assert((retInMemPointer != NULL && sysFunc->hostReturnInMemory) ||
         (retInMemPointer == NULL && !sysFunc->hostReturnInMemory));
