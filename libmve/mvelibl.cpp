...
static bool sndConfigure(unsigned rate, unsigned buflen, unsigned stereo, unsigned frequency, unsigned bits16, unsigned comp16)
{
#if SOUND_SUPPORT
	if (!snd_ds)
		return true;
...
static void sndAdd(unsigned char *buf, unsigned len)
{
	...
	dsrval = snd_buffer->Lock(write_cursor,len,(void **)&ptr1,&len1,(void **)&ptr2,&len2, 0/*LNXSND_LOCK_FROMWRITECURSOR*/ /*flags*/);
	if (dsrval != 0)
		return;
...
unsigned char* nf_buf_cur = NULL;
unsigned char* nf_buf_prv = NULL;
static MemRec nf_mem_buf1;
static MemRec nf_mem_buf2;

// NextFrame parameters
unsigned char nf_wqty   = 0;	 // (width/SWIDTH)
unsigned char nf_hqty   = 0;	 // (height/SHEIGHT)
unsigned char nf_fqty   = 0;	 // Number of fields
...

	{
		if (hicolor)
		{
			...
			for (i=0, r=0, b=0; r<32; ++r)
			{
				...
				{
					nf_trans16_hi[i+128] =
					nf_trans16_hi[i] = (((pal_rshift>0?r<<pal_rshift:r>>-pal_rshift)&pal_rmask) |
										((pal_gshift>0?g<<pal_gshift:g>>-pal_gshift)&pal_gmask) |
										((pal_bshift>0?b<<pal_bshift:b>>-pal_bshift)&pal_bmask) );
				}
			}
		...
		nf_buf_cur = (unsigned char *)malloc(size);
		nf_buf_prv = (unsigned char *)malloc(size);
	}
...
//
void nfHiColorDecompChg( const unsigned short *chgs, const unsigned short *parms, const unsigned char *comp, unsigned x, unsigned y, unsigned w, unsigned h );
...
// Parms are motion parms for squares to update.
//
void nfDecompChg(const unsigned short *chgs, const unsigned short *parms, const unsigned char *comp, unsigned x, unsigned y, unsigned w, unsigned h);
...
unsigned char *sf_WriteWinLimit     = NULL;
...
static unsigned char *rm_p        = NULL;
