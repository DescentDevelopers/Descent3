...
void PkDecompWorker( const bool hiColor, const unsigned char *ops, const unsigned char *comp, const unsigned x, const unsigned y, const unsigned w, const unsigned h );
...
void DOnf_xypshift( const bool hiColor, const unsigned int eax, unsigned char *&edi, const int nfpk_back_right, const int DiffBufPtrs );
...
unsigned short Trans16( const unsigned char* idx )
{
	return nf_trans16_lo[ idx[0] ] | nf_trans16_hi[ idx[1] ];
}

int SkipOpcode( int opcode_to_use, bool hiColor, const unsigned char* &esi, const unsigned char* &bcomp, unsigned char* &edi )
{
	...
	case 9:
		{
			...
			if( mode2 )      esi += (hiColor)?16:12;
			else if( mode3 ) esi += (hiColor)?12:8;
			else             esi += (hiColor)?24:20;
		}break;
	case 10:
		{
...
void PkDecompWorker( const bool hiColor, const unsigned char *ops, const unsigned char *comp, const unsigned x, const unsigned y, unsigned w, unsigned h )
{
#ifdef OUTRAGE_BIG_ENDIAN
		...
		case -1:
			{
				...
				else
				{
					...
					const unsigned int  kPalIndexDWord = (kPaletteIndex << 24) | (kPaletteIndex << 16) | (kPaletteIndex << 8) | kPaletteIndex;
					for( int r = 0; r < 8; ++r )
					{
		...
		case 7:
			{
				...
				else
				{
					...
					for( int rep_count = 0; rep_count < max_repcount; ++rep_count )
					{
						...
						switch(rep_count)
						{
						case 0: idx = esiOffset[0] & 0xF;  break;
						case 1: idx = esiOffset[0]>>4;     break;
						case 2: idx = esiOffset[1] & 0xF;  break;
						case 3: idx = esiOffset[1]>>4;     break;
						}
		...
		case 8:
			{
				...
				if( !donf24 && !donf40 )
				{
					...
					if( !hiColor )
					{
						repLookup[0] -= 2; repLookup[1] -= 2;
						repLookup[2] -= 4; repLookup[3] -= 4;
						repLookup[4] -= 6; repLookup[5] -= 6;
						repLookup[6] -= 8; repLookup[7] -= 8;
					}
					const unsigned char* lookupTable = ( hiColor ) ? nfhpk_mov8 : nfpk_mov8;
				...
				{						
					...
					if( !hiColor )
					{
						repLookupTable[0] -= 2; repLookupTable[1] -= 2;
						repLookupTable[2] -= 2;	repLookupTable[3] -= 2;
						repLookupTable[4] -= 4; repLookupTable[5] -= 4;
						repLookupTable[6] -= 4;	repLookupTable[7] -= 4;
					}
...
void DOnf_shift( const bool hiColor, const int offset, unsigned char * &edi, const int nfpk_back_right )
{
	...
	ptr dstBuffer; dstBuffer.pAsChar = edi;
	ptr srcBuffer; srcBuffer.pAsChar = edi + offset;
...
void DECOMP_BODY( bool HI_COLOR_FLAG, const unsigned char* &comp, unsigned int _x, unsigned int _y, unsigned int _w, unsigned int _h )
{
...
void DECOMP_CHG_BODY(bool HI_COLOR_FLAG,
					 const unsigned short* &chgs, const unsigned short* &parms,
