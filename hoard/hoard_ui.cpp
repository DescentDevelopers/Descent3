...
#define WINDOW_W	352
	...
	// text items
	// ----------
	void *start_game_text_off	= DLLCreateNewUITextItem(TXT_RETURNTOGAME,UICOL_HOTSPOT_LO);
	void *start_game_text_on	= DLLCreateNewUITextItem(TXT_RETURNTOGAME,UICOL_HOTSPOT_HI);
	void *window_title			= DLLCreateNewUITextItem(TXT_GAMECONFIG,UICOL_WINDOW_TITLE);
	void *min_count_label_ti	= DLLCreateNewUITextItem(TXT_MINIMUMORBS,UICOL_TEXT_NORMAL);
	void *slider_val_ti			= DLLCreateNewUITextItem(buffer,UICOL_TEXT_NORMAL);
	...
	while (!exit_menu) 
	{
		...
		case 31:
			{//update slider
				...
				slider_val_ti = DLLCreateNewUITextItem(buff,UICOL_TEXT_NORMAL);
