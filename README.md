...
1. **Install the prerequisite build tools.**
    ...
    **NOTE:**
    Builds _must_ be performed in one of the Visual Studio-provided command prompts ("Developer Command Prompt", "Developer PowerShell", "x64 Native Tools Command Prompt", etc), _not_ a standard Command Prompt or PowerShell. The VS prompts will already have vcpkg installed and configured for use, so no dependency management is needed.
