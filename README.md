...
1. **Install the prerequisite build tools.**
    ...
    **NOTE:**
    Builds _must_ be performed in the “x64 Native Tools Command Prompt” (or PowerShell) provided by Visual Studio, _not_ a standard Command Prompt (or PowerShell). The Native Tools CLIs will already have vcpkg installed and configured for use, so no dependency management is needed.
...
1. **Install the prerequisite build tools.**
    ...
    * APT users (Debian, Ubuntu)
        ```sh
        sudo apt update
        sudo apt install -y --no-install-recommends git ninja-build cmake g++ libncurses-dev
        ```
    * DNF users (Red Hat, Fedora)
        ```sh
        sudo dnf update --refresh
        sudo dnf install -y git ninja-build cmake gcc-c++ ncurses-devel
        ```
...
2. **Acquire the library dependencies.**
    ...
    * If you would like to use vcpkg:
        ...
        2. Install vcpkg-needed build tools and dependencies:
            * APT users
                ```sh
                sudo apt install -y --no-install-recommends curl pkg-config autoconf automake libtool libltdl-dev make python3-jinja2 libx11-dev libxft-dev libxext-dev libwayland-dev libxkbcommon-dev libegl1-mesa-dev libibus-1.0-dev
                ```
            * DNF users
                ```sh
                sudo dnf install -y autoconf automake libtool perl-open perl-FindBin python-jinja2 libX11-devel libXft-devel libXext-devel wayland-devel libxkbcommon-devel mesa-libEGL-devel ibus-devel
                ```
    * If you would like to manage the code dependencies yourself:
        * APT users
            ```sh
