...
//	create new surface: set up surface structure
	...
	switch (type)
	{
		case SURFTYPE_VIDEOSCREEN:
			grerr = ddgr_surf_InitVideo(&ddsfObj); break;
		case SURFTYPE_GENERIC:
			grerr = ddgr_surf_Create(&ddsfObj); break;
		case SURFTYPE_MEMORY:
			grerr = (bool)gr_mem_surf_Create(&ddsfObj); break;
		default:
			Int3();					// Invalid!!
	}
	if (!grerr) Int3();
...
/*	Freeing surface is same regardless of whether it was loaded from a bitmap handle or
	created ourselves 
*/
	if (!m_SurfInit) return;
	ASSERT(!surf_Locked);

	switch (ddsfObj.type)
	{
		case SURFTYPE_VIDEOSCREEN:
			ddgr_surf_CloseVideo(&ddsfObj); break;
		case SURFTYPE_GENERIC:
			ddgr_surf_Destroy(&ddsfObj); break;
		case SURFTYPE_MEMORY:
			gr_mem_surf_Destroy(&ddsfObj); break;
		default:
...
/*	perform lock, then copy data to this surface, then unlock */
	...
	{
		case BPP_16:
			if (bpp == BPP_16) xlat16_16(data, w, h,format);
			if (bpp == BPP_24) xlat24_16(data, w, h);
			if (bpp == BPP_32) xlat32_16(data, w, h);
			break;
			
		case BPP_24:
			if (bpp == BPP_16) xlat16_24(data, w, h);
			break;					   
...
void grSurface::clear(ddgr_color col)
{
	grSurface::clear(0,0,ddsfObj.w, ddsfObj.h, col);
}
...
void grSurface::clear(int l, int t, int w, int h, ddgr_color col)
{
	...
	switch (ddsfObj.type)
	{
		...
		//	---------------------------------------------------------------------------
			...
			break;
		case SURFTYPE_MEMORY:
			gr_mem_surf_Clear(&ddsfObj, col,l, t, w, h); break;
		default:
			Int3();
	}
}
...
char *grSurface::lock(int *rowsize)
{
	...
	switch(ddsfObj.type)
	{
		case SURFTYPE_VIDEOSCREEN:
		case SURFTYPE_GENERIC:
			grerr = ddgr_surf_Lock(&ddsfObj, (void **)&m_OrigDataPtr, &m_DataRowsize); break;
		case SURFTYPE_MEMORY:
			grerr = gr_mem_surf_Lock(&ddsfObj, (void **)&m_OrigDataPtr, &m_DataRowsize); break;
		default:  
			Int3();						// invalid type!!
	}
...
/*	calculate the adjusted data pointer based off of x,y,bpp */
	...
	switch(ddsfObj.bpp)
	{
		case BPP_16: data += (x*2); break;
		case BPP_32: data += (x*4); break;
		default: Int3();
	}
...
void grSurface::unlock()
{
	...
	switch(ddsfObj.type) 
	{
		case SURFTYPE_VIDEOSCREEN:
		case SURFTYPE_GENERIC:
			grerr = ddgr_surf_Unlock(&ddsfObj, m_OrigDataPtr); break;
		case SURFTYPE_MEMORY:
			grerr = gr_mem_surf_Unlock(&ddsfObj, m_OrigDataPtr); break;
		default:  
			Int3();						// invalid type!!
	}
