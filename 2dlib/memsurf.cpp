...
grMemorySurface::grMemorySurface(int bm)
{
	...
	grMemorySurface::init(bm_w(bm,0), bm_h(bm,0), bm_bpp(bm), (char *)bm_data(bm, 0), bm_rowsize(bm,0), SURFFLAG_COLORKEY);
}
...
bool grMemorySurface::init(int w, int h, int bpp, char *data, int rowsize, unsigned flags, const char *name)
{
	...
	if (!gr_mem_surf_Init(&ddsfObj, data, rowsize)) return 0;
...
bool gr_bm_mem_Blt16(ddgr_surface *dsf, 
							int dx, int dy, 
							ddgr_surface *ssf, 
							int sx, int sy, int sw, int sh);
bool gr_bm_mem_Blt16ck(ddgr_surface *dsf, 
							int dx, int dy, 
							ddgr_surface *ssf, 
							int sx, int sy, int sw, int sh);
...
bool gr_mem_surf_AttachHandle(ddgr_surface *sf, unsigned handle)
{
	return true;
}
