...
//@@											ft->ch_uf[cbi->ch], ft->ch_vf[cbi->ch], 
...
//@@		return (cbi->x + ft->ch_w[cbi->ch] - 1);
		...
		rend_DrawFontCharacter (ft->bmps[ft->ch_bmp[cbi->ch]], cbi->x,cbi->y,
											...
											(int)(cbi->y+cbi->sh),
											ft->ch_uf[cbi->ch]+(((float)cbi->sx)/((float)GRFONT_SURFACE_WIDTH)), 
											ft->ch_vf[cbi->ch]+(((float)cbi->sy)/((float)GRFONT_SURFACE_HEIGHT)), 
											((float)cbi->sw)/((float)GRFONT_SURFACE_WIDTH), 
											((float)cbi->sh)/((float)GRFONT_SURFACE_HEIGHT));
