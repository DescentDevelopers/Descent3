...
tSerialPort ddio_SerialOpenPort(int port_number, int baud)
{
    ...
    else if (baud == 2400)    _baud = B2400;
    else if (baud == 4800)    _baud = B4800;
    else if (baud == 9600)    _baud = B9600;
    else if (baud == 19200)   _baud = B19200;
    else if (baud == 38400)   _baud = B38400;
    else if (baud == 57600)   _baud = B57600;
    else if (baud == 115200)  _baud = B115200;
    else return(NULL);
...
bool ddio_SerialWriteByte(tSerialPort port, ubyte b)
{
    ...
    if (retVal == false)
    {
        mprintf((0, "DDIO: Writing byte (%u) to descriptor (%d) failed!",
                    (unsigned int) b, fd));
    } // if
