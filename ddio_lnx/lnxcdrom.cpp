...
void ddio_ReinitCDSystem(void)
{
    CDROM_Devices.reinit();
}
...
void cdrom_system_shutdown(void)
{
	CDROM_Devices.ResetMountpoint();
}
...
int system_cdroms::GetNumCDRoms(void)
{
    return(SDL_CDNumDrives());
}

// Returns the device name of the default drive
const char *system_cdroms::GetDeviceName(void)
{
	return CDName(m_DeviceToUse);
}

// Returns the volume name of the default drive
const char *system_cdroms::GetVolumeName(void)
{
	return GetVolume(m_DeviceToUse);
}

const char *system_cdroms::CDName(int drive)
{
...
// Returns true if a CD is in the default device drive
bool system_cdroms::IsCdInDrive(void)
{
	return IsCdInDrive(m_DeviceToUse);
}
...
static bool checkDevMount(const char *device, const char *mntfname, char *mountpoint)
{
    ...
    if (mountFile == NULL)
    {
        mprintf((0, "setmntent(\"%s\", \"r\"); failed!  errno == (%d).",
                    mntfname, errno));
        Int3();
        return(false);
    } // if
...
void system_cdroms::FindDefaultDevice(void)
{
    ...
    else  // there WAS at least one drive...
    {
    	...
    	mprintf((0,"CDROM: Making device \"%s\" default CD-ROM drive.",
                 CDName(m_DeviceToUse)));
    } // else
