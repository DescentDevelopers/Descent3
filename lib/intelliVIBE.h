...
typedef struct
{
	...
	int game_paused;   // 1 if the game is currently paused, 0 if it isn't.  Note: if the game is paused, Gametime and Frametime will be invalid
}d3_frame_info;
...
typedef struct
{
	...
	float shake_magnitude;   // If the player's ship is shaking due to some external force, it will be in this value, 0<=magnitude<=120
