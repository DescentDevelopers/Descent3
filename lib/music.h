...
{
	ubyte cmd;
	union { float f; int i; void *p; } parm;
};
...
public:
	...
	void SEND_STRM_NEXT(const char *fname, float vol, ubyte count,bool *switch_flag);	// specifies next stream to play in current stream.
...
//	takes a OMS theme file.
	...
	{
		return m_sequencer_init && m_sequencer_run;
	}
