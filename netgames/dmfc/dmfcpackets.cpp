...
void DMFCBase::SendRemoteKey(int pnum)
{
	...
	{		
		...
		memcpy(&data[count],key,8); count+=8;
...
void DMFCBase::GetRemoteKey(ubyte *data)
{
	int count = 0;
	ubyte key[8];
	memcpy(key,&data[count],8); count+=8;
	Remote_SetMyKey(key);
}
...
void DMFCBase::ReceiveNetGameInfoSync(ubyte *data)
{
	...
	if(MultiGetByte(data,&count)) Netgame->flags |= NF_KILLGOAL; else Netgame->flags &= ~NF_KILLGOAL;
	if(MultiGetByte(data,&count)) Netgame->flags |= NF_TIMER; else Netgame->flags &= ~NF_TIMER;
