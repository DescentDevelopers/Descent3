...
void DMFCBase::OnClientPlayerKilled(object *killer_obj,int victim_pnum)
{
	...
	if(killer_obj && killer_obj->type==OBJ_PLAYER && victim_pnum!=-1 ){
		...
		if(	Players[killer_obj->id].team == Players[victim_pnum].team &&
				(GetNumTeams()>=2) && (killer_obj->id!=victim_pnum) ) {
...
void DMFCBase::OnClientLevelEnd(void)
{
	...
	for(int i=0;i<DLLMAX_PLAYERS;i++){
		if( (NetPlayers[i].flags&NPF_CONNECTED) && (NetPlayers[i].sequence>=NETSEQ_PLAYING || NetPlayers[i].sequence==NETSEQ_WAITING_FOR_LEVEL) )
		{
...
void DMFCBase::OnSaveStatsToFile(void)
{
	mprintf((0,"User requested stats to be saved!\n"));
}
...
void DMFCBase::OnPLRInit(void)
{
	DISABLE_FLAGS(m_iProtectedFlags,DMFC_PRF_PLRFIRSTTIME);
}
...
bool DMFCBase::OnAllowObserverChange(bool turnonobserver)
{
	return true;
}
...
void DMFCBase::OnClientWallCollide(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot)
{
}
