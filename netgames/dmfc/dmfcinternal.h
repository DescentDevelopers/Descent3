...
public:
	MenuItem();
	MenuItem(char *title,char type,ubyte flags,void (*fp)(int), ...);	//the last parameters are the state items, which is 
																		//an int (the number of states), followed by 
...
public:
	...
	virtual void OnServerWallCollide(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot);
	...
	//
	//	Called by the game when their is a collision between an object and a wall
	virtual void OnClientWallCollide(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot);
	...
	int SetupPlayerRecord(int sizeof_individual_data,int (*pack_callback)(void *user_info,ubyte *data),int (*unpack_callback)(void *user_info,ubyte *data));
...
public:
	...
	void Set_OnServerWallCollide(void (*callback)(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot));
	...
	void Set_OnClientWallCollide(void (*callback)(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot));
	...
	void CallOnServerWallCollide(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot);
	...
	void CallOnClientWallCollide(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot);
...
private:
	...
	void (*UserOnServerWallCollide)(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot);
	...
	void (*UserOnClientWallCollide)(object *obj,float hitspeed,int hitseg,int hitwall,vector * hitpt,vector *wall_normal,float hit_dot);
...
#define DMFC_PRF_NOTEAMSELECTONSTART	0x00001000	// Enabled: DMFC should NOT display the Team Select dialog on startup of a team game level
#define DMFC_PRF_SHIPLOGOSENABLED	0x00002000	// Enabled: Display ship logos
#define DMFC_PRF_NOWAITFORPLAYERS	0x00004000	// Enabled: DMFC should NOT display the wait for players dialog on startup of a single player game
...
#define DTXT_IC_KILLMSGFILTERD		DTXT(115)	//"Sets the kill message filter, for what style of messages you want.\nUsage: \"$killmsgfilter <full/simple/none>\""
...
#define DTXT_IC_OBSERVERD			DTXT(119)	//"If you pass specify 'on', it puts you into observer mode, else it will return you back to normal mode.\nUsage: \"$observer <on/off>\""
...
#define DTXT_IC_HEADERINSTRUCT		DTXT(153)	//"Prefix a '$' before the commands listed below.  To get more detailed help\nabout a command, type '$help <command>'\n"
...
//				-1 out of memory (all user stats memory will be freed)
//				-2 invalid callbacks
int PRec_SetupUserPRec(int sizeof_user_stats,int (*pack_callback)(void *user_info,ubyte *data),int (*unpack_callback)(void *user_info,ubyte *data));
