...
void PlayerStack::Add(char *callsign,network_address *addr,tDMFCPlayerStat *pstat,void *GameStatItem,int GameStatItemsize)
{
	if(!callsign)
		return;
	if(!addr)
...
bool PlayerStack::Find(char *callsign,network_address *addr,tDMFCPlayerStat *stat,bool remove,void *GameStatItem,int GameStatItemsize)
{
	if(!root){
