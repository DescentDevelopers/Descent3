...
void PRec_SendPRecToPlayer(int pnum)
{
	...
	for(int i=0;i<MAX_PLAYER_RECORDS;i++){
		if(Player_records[i].state!=STATE_EMPTY){
			...
			memcpy(&data[count],Player_records[i].callsign,callsignlen); count+=callsignlen;//pack bytes (callsign)
...
void PRec_ReceivePRecFromServer(ubyte *data)
{
	...
	memcpy(pr->callsign,&data[count],callsignlen); count+=callsignlen;	//unpack bytes (callsign)
