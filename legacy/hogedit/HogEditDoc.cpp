...
#ifdef _DEBUG
void CHogEditDoc::AssertValid() const { CDocument::AssertValid(); }

void CHogEditDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
//	saves a HOG file.
int CHogEditDoc::SaveHog(const char *pathname) { return FALSE; }
...
bool CHogEditDoc::CreateFilenameList(char ***filenames) {
  ...
  *filenames = new (char(*[Library.num_entries]));
  ...
  while (pos != NULL) {
    ...
    if ((*filenames)[j] == NULL) {
      OutputDebugString("Outta memory!\n");
      return FALSE;
    }
...
int CHogEditDoc::CreateNewHogFromCurrentHog(char *src_hog_fname, char *target_hog_fname, int nfiles,
                                            const char **filenames, void (*UpdateFunction)(char *)) {
  unsigned i;
  int table_pos;
  FILE *hog_fp;
  tHogHeader header;
