...
int16_t LevelScript_0000::CallEvent(int event, tOSIRISEventInfo *data) {
  ...
  case EVT_LEVELSTART: {
    ...
    // Script 029: IntroCam
    if (1) {
      aCinematicIntro(Path_indexes[2], TXT("IntroMessage"), Object_handles[2], Path_indexes[3], 12.000000f);
...
int16_t CustomObjectScript_10DC::CallEvent(int event, tOSIRISEventInfo *data) {
  ...
  case EVT_COLLIDE: {
    ...
    if (qObjIsPlayer(event_data->it_handle) == true) {
      ...
      aObjectPlayerGiveKey(event_data->it_handle, Object_handles[13], 1, TXT("ClassifiedAccessPass"), 1);
