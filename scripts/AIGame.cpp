...
private:
	...
	void RemapWB(int me, float start, float fire, int fire_sound, float end, float time, float latency, int index, unsigned short w_id, char f_mask);
	void DoFrame(int me);
	bool DoNotify(int me_handle, tOSIRISEventInfo *data);
	void SetMode(int me, char mode);
public:
...
int gb_command_text[NUM_GB_COMMANDS] =
{
	...
	TXT_GBM_ANTIVIRUS,
	TXT_GBM_ALLOWCHAT,
	TXT_GBM_DISALLOWCHAT,
	TXT_GBM_RETURNTOSHIP,
};
...
int gb_pow_text[NUM_GB_POWERUPS] =
{
	TXT_GBP_PUTOUTFIRE,
	TXT_GBP_COWABUNGA,
	TXT_GBP_BEGENTAL,
...
int gb_pow_not_text[NUM_GB_POWERUPS] =
{
	TXT_GBPN_NOFIRE,
	TXT_GBPN_NOFIGHT,
	TXT_GBPN_GOPLAY,
...
int gb_pow_pickup_text[NUM_GB_POWERUPS] =
{
	TXT_GBPP_EXTIN,
	TXT_GBPP_WINGNUT,
	TXT_GBPP_MANCON,
...
void SuperThief::SpewEverything(int me)
{
	...
	for(i = 0; i < memory->num_stolen_weapons; i++)
	{
		...
		for(j = 0; j < memory->stolen_weapons[i].amount; j++)
		{
			...
			Player_Value(memory->stolen_weapons[i].owner, VF_GET, PLYSV_US_WEAPON_POWERUP_ID, &id, memory->stolen_weapons[i].index);
...
void SuperThief::CheckAndFireSecondary(int me)
{
	...
	for(i = 0; i < memory->num_stolen_weapons; i++)
	{
		if(memory->stolen_weapons[i].amount > 0 && SuperThiefableItems[memory->stolen_weapons[i].index].type == THIEFABLEITEM_SECONDARY)
		{
...
void SuperThief::DoInterval(int me)
{
	...
	{
		// Do dynamic death
		if((memory->mode_time < 4.0f && memory->mode_time + Game_GetFrameTime() >= 4.0f) || (memory->mode_time < 6.0f && memory->mode_time + Game_GetFrameTime() >= 6.0f))
		{
			int weapon_id;
		...
		{
			mprintf(0, "ST: Died\n");
			Obj_Kill(me, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_LARGE | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
		}
	}
	...
	else
	{
		...
		if(memory->mode == SUPERTHIEF_RANGED)
		{
			...
			if(memory->next_fire_flare_time < Game_GetTime() && last_see_time + 7.0f >= Game_GetTime() && last_hear_time + 7.0f >= Game_GetTime())
			{
				FireFlare(me);
...
void DestroyOnContact::DoCollide(int me, int it)
{
	...
	{
		Obj_Kill(it, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
	}
}
...
short Pest::CallEvent(int event, tOSIRISEventInfo *data)
{
	switch(event)
	{
		...
		case EVT_AI_FRAME:
		{
			...
			if((frame > 9 && memory->last_anim_frame <= 9) ||
				(frame > 10 && memory->last_anim_frame <= 10) ||
...
bool Humonculous::SetMode(int me, unsigned short mode)
{
	...
	switch(mode)
	{
		...
		case HM_WALL_HIT:
		{
			...
			AI_AddGoal(me, AIG_GET_TO_POS, 1, 1.0, H_GUID_LANDED, GF_ORIENT_SCRIPTED | GF_USE_BLINE_IF_SEES_GOAL | GF_NOTIFIES, &memory->land_pos, ray.hit_room);
...
//	mprintf(0, "Anim frame is %f\n", frame);
	...
	switch(memory->mode)
	{
		...
		case HM_WALL_HIT:
		{
			...
			if(memory->mode_time > 4.0f && anim_type != AS_CUSTOM && next_anim_type != AS_CUSTOM && (memory->flags & HF_HIT_WALL))
			{
				SetMode(me, HM_RANGED);
			}
			else if(memory->mode_time > 15.0f && anim_type != AS_CUSTOM)  // Safety case - if there is a messup, return to ranged (worked fine)
			{
				SetMode(me, HM_RANGED);
			}
		}
		...
		case HM_ABOUT_TO_FAKE_DEATH:
		{
			if(memory->flags & HF_AT_FDPOS)
			{
				if(memory->mode_time < 6.0f && memory->mode_time + Game_GetFrameTime() >= 6.0f)
				{
					...
					int flags = FQ_CHECK_OBJS | FQ_IGNORE_POWERUPS | FQ_IGNORE_WEAPONS | FQ_IGNORE_MOVING_OBJECTS | FQ_IGNORE_NON_LIGHTMAP_OBJECTS;
					int fate = FVI_RayCast(me, &start_pos, &end_pos, start_room, 0.0f, flags, &ray);
...
short Dragon::CallEvent(int event, tOSIRISEventInfo *data)
{
	switch(event)
	{
		...
		case EVT_DESTROY:
		{
			...
			Obj_Kill(memory->head_object, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
			Obj_Kill(memory->tail_object, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
			Obj_Kill(memory->green_turret, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
			for(i = 0; i < 6; i++)
			{
				Obj_Kill(memory->t[i], OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
				Obj_Kill(memory->tentacle[i], OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
			}
		}
		break;
	}
...
short Flak::CallEvent(int event, tOSIRISEventInfo *data)
{
	switch(event)
	{
		...
		break;
		case EVT_DESTROY:
			Obj_Kill(memory->canopy_handle, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);		
			break;
		case EVT_MEMRESTORE:
		{
		...
		case EVT_INTERVAL:
		{
			if(!(memory->flags & JUGG_F_BELLY_DEAD))
			{
				...
				if(type == OBJ_NONE)
				{
					...
					for(i = 0; i < 3; i++)
					{
						Obj_Kill(memory->flame_turret[i], OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
					}
				}
			}
		}
...
void DTower::DoFrame(int me)
{
	...
	else if(memory->mode_time < 3.0f && memory->mode_time + Game_GetFrameTime() >= 3.0f)
	{
		...
		Obj_Kill(memory->gun_handle, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_LARGE | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
...
void DCollector::DoFrame(int me)
{
	if(!memory->f_dead)
	{
		...
		{
			memory->f_dead = true;
			Obj_Kill(memory->rod_handle, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
...
void CBall::DoFrame(int me)
{
	...
	else if(memory->f_picked_up && !f_picked_up)
	{
		...
		{
			Obj_Kill(me, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_MEDIUM | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
		}
	}
}
...
bool SixGun::DoNotify(int me, tOSIRISEVTAINOTIFY *notify)
{
	if(memory->mode == 0) 
	{
		if((notify->notify_type == AIN_OBJ_FIRED) || (notify->notify_type == AIN_NEAR_TARGET) || (notify->notify_type == AIN_HIT_BY_WEAPON) || (notify->notify_type == AIN_BUMPED_OBJ)) 
		{
			int flags = DWBF_ENABLED;
			...
			AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ, 0, 1.0f, -1, GF_OBJ_IS_TARGET | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, OBJECT_HANDLE_NONE);
			memory->mode = 1;
		}
	}
...
void GuideBot::ReInitAmbient(int me)
{
	...
	AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 0, 1.0f, -1, GF_KEEP_AT_COMPLETION | GF_USE_BLINE_IF_SEES_GOAL, memory->amb_camera_handle, dir_index);
...
bool GuideBot::SetMode(int me, char mode)
{
	...
	float	dist = 4.0f;
	int g_index = AI_AddGoal(me, AIG_GET_AROUND_OBJ, ACTIVATION_BLEND_LEVEL, 1.0f, -1, GF_OBJ_IS_TARGET, OBJECT_HANDLE_NONE);
...
//			AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 0, 1.0f, -1, GF_KEEP_AT_COMPLETION, memory->my_player, GST_FVEC);
//			AI_SetGoalCircleDist(me, 0, 40.0f);
		...
		case GBM_RTYPE:
		{
			...
			AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 3, 1.0f, -1, GF_KEEP_AT_COMPLETION | GF_ORIENT_SCRIPTED, memory->camera_obj, GST_NEG_FVEC);
//			AI_SetGoalCircleDist(me, 3, 4.0f);
		...
		case GBM_EXTINGUISH:
			...
			AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 0, 1.0f, -1, GF_KEEP_AT_COMPLETION | GF_ORIENT_GOAL_OBJ, memory->extinguish_obj_list[0], 0);
...
int GuideBot::MakeCommandList(int *gbc_list)
{
	...
	else if(memory->mode == GBM_AMBIENT)
	{
		...
		for(i = GBC_FIND_ACTIVE_GOAL_0; i < NUM_GB_COMMANDS; i++)
		{
			...
			{
				if(Obj_MakeListOfType(memory->me, OBJ_MARKER, i - GBC_FIND_MARKER_0, false, OBJECT_HANDLE_NONE, 1, &memory->g_powerup) == 0)
					continue;
			}
...
bool GuideBot::DoNotify(int me, tOSIRISEventInfo *data)
{
	...
	else if(IsGoalFinishedNotify(notify->notify_type)) // Goal complete -- chrishack -- print something else on error cases
	{
...
void GuideBot::DoFrame(int me)
{
	...
	else if(memory->mode == GBM_EXTINGUISH)
	{
		...
		if (Game_GetTime() > memory->next_ambient_time) 
		{
			...
			int dir_index = rand()%6;
			AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 0, 1.0f, -1, GF_ORIENT_GOAL_OBJ | GF_KEEP_AT_COMPLETION, memory->extinguish_obj_list[0], dir_index);
...
bool Thief::DoSteal(int me, int it, int attempt_num, bool f_last_success)
{
	...
	while(max_tries--)
	{
		...
		perc_chance = (attempt_num==1)?ThiefableItems[i].attempt_one:(f_last_success)?ThiefableItems[i].attempt_two:ThiefableItems[i].attempt_two_no_one;
...
void Thief::SetMode(int me, int new_mode)
{
	...
	switch(new_mode)
	{
		case THIEF_AMBIENT:
		{
			AI_AddGoal(me, AIG_WANDER_AROUND, 2, 1.0f, -1, GF_ORIENT_VELOCITY | GF_NOTIFIES | GF_KEEP_AT_COMPLETION, &pos, room);
			AI_SetGoalFlags(me, 2, GF_ORIENT_TARGET, false);
		}
		break;
		case THIEF_STEAL:
...
//summon: 12-29 (3.0 seconds)  19-23 (summons)
	...
	if((flags & OF_AI_DEATH) && !((anim_frame >= 0.0f && anim_frame <= 6.0f) || (anim_frame >= 85.0f && anim_frame <= 140.0f)))
	{
		int room;
		vector pos;
		int weapon_id;
...
void Thief::DoDestroy(int me, tOSIRISEVTDESTROY *evt_destroy)
{
	SpewEverything(me);
}
...
void Sickle::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		...
		case SICKLE_LAND_ON_CEILING:
		{
			...
			AI_AddGoal(me, AIG_GET_TO_POS, 2, 1.0f, -1, /*GF_NONFLUSHABLE | GF_KEEP_AT_COMPLETION |*/ GF_ORIENT_SCRIPTED, &memory->ceiling_pos, ceiling_room);
...
bool Sickle::DoNotify(int me_handle, tOSIRISEVTAINOTIFY *notify)
{
	if((IsGoalFinishedNotify(notify->notify_type)) && 
		(memory->mode == SICKLE_GOING_HOME || memory->mode == SICKLE_LAND_ON_CEILING) &&
		(notify->goal_num == 2))
	{
		...
		if(memory->time_till_pulse <= 0.0f) 
		{
			...
			for(i = 0; i < n_scan; i++)
			{
				...
				if(num_wbs > 0 && ctype == CT_AI)
				{
					...
					// If it is a PTMC bot
					if(((flags & AIF_TEAM_MASK) == AIF_TEAM_PTMC) && !(t_type == OBJ_PLAYER && Game_GetTime() - last_see_target_time < 7.0f))
					{
						float fov;
...
//#define BSCOMM_FOLLOW_ME								0 // send a percent chance so we can have a line sometimes
//#define BSCOMM_FOLLOW_CANCEL_FROM_FOLLOWER		2
//#define BSCOMM_FOLLOW_CANCEL_FROM_LEADER			3
...
void BarnSwallow::ComputeNest(int me)
{
	...
	memory->nest_center = pos; // Each robot will have a different center point becuase thier initial positions are weighted twice
...
//		return false;
	...
	switch(mode)
	{
		...
		case BSM_ATTACK:
		{
			...
			AI_AddGoal(me, AIG_GET_TO_OBJ, 1, 1.0f, -1, GF_SPEED_ATTACK | GF_ORIENT_VELOCITY | GF_OBJ_IS_TARGET | GF_USE_BLINE_IF_SEES_GOAL, OBJECT_HANDLE_NONE);
			AI_GoalValue(me, 1, VF_SET, AIGV_F_CIRCLE_DIST, &dist);

			dist = 40.0f;
			int g_index = AI_AddGoal(me, AIG_GET_AROUND_OBJ, ACTIVATION_BLEND_LEVEL, 1.0f, -1, GF_ORIENT_VELOCITY | GF_OBJ_IS_TARGET, OBJECT_HANDLE_NONE);
			AI_GoalValue(me, g_index, VF_SET, AIGV_F_CIRCLE_DIST, &dist);
		}
		break;
		case BSM_ATTACK_CIRCLE_BACK:
		{
			...
			AI_AddGoal(me, AIG_WANDER_AROUND, 1, 1.0f, -1, GF_SPEED_FLEE | GF_ORIENT_VELOCITY | GF_NOTIFIES | GF_KEEP_AT_COMPLETION, &pos, room);
		...
		case BSM_RETURN_TO_NEST:
		{
			AI_AddGoal(me, AIG_GET_TO_POS, 1, 1.0, -1, GF_SPEED_FLEE | GF_ORIENT_VELOCITY | GF_USE_BLINE_IF_SEES_GOAL, &memory->nest_center, memory->home_room);
...
void Hellion::RemapWB(int me, float start, float fire, int fire_sound, float end, float time, float latency, int index, unsigned short w_id, char f_mask)
{
	memory->start = start;
	memory->fire = fire;
	memory->end = end;
...
void Hellion::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		...
		case HELLION_MATTER:
			...
			RemapWB(me, 100.0f, 120.0f, Sound_FindId("fragA2"), 120.0f, 2.5f, 0.5f, 0, Wpn_FindID("HellionFrag"), (1<<1)|(1<<2)|(1<<3)|(1<<4));
		...
		case HELLION_ENERGY:
			...
			RemapWB(me, 179.0f, 200.0f, Sound_FindId("Mega missile fire"), 200.0f, 2.7f, 0.9f, 0, Wpn_FindID("HellionMega"), (1<<1)|(1<<2)|(1<<3)|(1<<4));
...
void Hellion::DoFrame(int me)
{
	...
	else if(memory->mode == HELLION_DEATH)
	{
		...
		if(frame == 650.0f)
		{
			Obj_Kill(me, OBJECT_HANDLE_NONE, 1000.0f, DF_BREAKS_APART|DF_BLAST_RING|DF_LOSES_ANTIGRAV|DF_EXPL_LARGE|DF_CONTACT_BREAKS_APART|DF_CONTACT_BLAST_RING|DF_DEBRIS_SMOKES|DF_DEBRIS_FIREBALL|DF_DEBRIS_BLAST_RING, 0.0f, 0.0f);
		}
	}
...
//	}
	...
	{
		if(memory->mode == HELLION_MATTER || memory->mode == HELLION_ENERGY || memory->mode == HELLION_FIRE || memory->mode == HELLION_SLEEP)
		{
			int offset = (memory->mode - HELLION_MATTER + 1)%4;
			SetMode(me, HELLION_MATTER + offset);
		}
...
short Hellion::CallEvent(int event, tOSIRISEventInfo *data)
{
	switch(event)
	{
		...
		case EVT_DESTROY:
		{
			...
			Obj_Kill(memory->head_object, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_LARGE | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
			Obj_Kill(memory->turret_object, OBJECT_HANDLE_NONE, 1000.0f, DF_BLAST_RING | DF_LOSES_ANTIGRAV | DF_EXPL_LARGE | DF_FIREBALL | DF_BREAKS_APART | DF_DEBRIS_SMOKES, 0.0f, 0.0f);
		...
		case EVT_MEMRESTORE:
		{
			...
			RemapWB(data->me_handle, memory->start, memory->fire, snd, memory->end, memory->time, memory->latency, memory->index, wpn, memory->f_mask);
		}
		break;
	}
	return CONTINUE_CHAIN|CONTINUE_DEFAULT;
...
void MantaRay::SetMode(int me, char mode)
{
	...
	if(memory->flags & MRF_SQUADIE)
	{
		...
		AI_AddGoal(me, AIG_GET_TO_POS, 2, 1.0f, -1, GF_ORIENT_VELOCITY | GF_KEEP_AT_COMPLETION, &memory->goal_pos, memory->goal_room);
		float cd = .1f;
	...
	switch(mode)
	{
		case MRM_NORMAL:
		{
			...
			AI_Value(me, VF_CLEAR_FLAGS, AIV_I_FLAGS, &f_attack_flags);
			AI_AddGoal(me, AIG_WANDER_AROUND, 1, 1.0f, -1, GF_ORIENT_VELOCITY | GF_NOTIFIES | GF_KEEP_AT_COMPLETION, &pos, room);
			AI_GoalValue(me, 1, VF_SET, AIGV_F_CIRCLE_DIST, &dist);
		}
		break;
...
//			memory->next_attack_time = Game_GetTime() + 10.0 * ((float)rand()/(float)RAND_MAX);
			...
			dist = 10.0f;
			int g_index = AI_AddGoal(me, AIG_GET_AROUND_OBJ, ACTIVATION_BLEND_LEVEL, 1.0f, -1, GF_ORIENT_VELOCITY | GF_OBJ_IS_TARGET  | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, OBJECT_HANDLE_NONE);
			AI_GoalValue(me, g_index, VF_SET, AIGV_F_CIRCLE_DIST, &dist);
		}
		break;
		...
		case MRM_ATTACK_CIRCLE_BACK:
		{
			...
			AI_AddGoal(me, AIG_WANDER_AROUND, 1, 1.0f, -1, GF_NOTIFIES | GF_ORIENT_VELOCITY  | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, &pos, room);
			AI_GoalValue(me, 1, VF_SET, AIGV_F_CIRCLE_DIST, &dist);
...
//			memory->next_attack_time = Game_GetTime() + 10.0 * ((float)rand()/(float)RAND_MAX);
	...
	if(memory->flags & MRF_SQUADIE)
	{
		...
		AI_AddGoal(me, AIG_GET_TO_POS, 2, 4.0f, -1, GF_ORIENT_VELOCITY | GF_KEEP_AT_COMPLETION | GF_NONFLUSHABLE, &memory->goal_pos, memory->goal_room);
	}
...
void SpyHunter::SetMode(int me, char mode)
{
	...
	switch(mode)
	{
		case SHM_NORMAL:
		{
			...
			AI_AddGoal(me, AIG_WANDER_AROUND, 1, 1.0f, -1, GF_ORIENT_TARGET | GF_KEEP_AT_COMPLETION | GF_FORCE_AWARENESS | GF_NOTIFIES, &pos, room);
			AI_GoalValue(me, 1, VF_SET, AIGV_F_CIRCLE_DIST, &dist);
		}
		break;
		case SHM_ATTACK:
		...
		case SHM_BOMB_ATTACK:
		{
			...
			AI_AddGoal(me, AIG_GET_TO_OBJ, 1, 1.0f, -1, GF_ORIENT_VELOCITY | GF_KEEP_AT_COMPLETION | GF_OBJ_IS_TARGET | GF_FORCE_AWARENESS, OBJECT_HANDLE_NONE);
			AI_GoalValue(me, 1, VF_SET, AIGV_F_CIRCLE_DIST, &dist);
		}
		break;
	}
...
bool SpyHunter::DoNotify(int me, tOSIRISEventInfo *data)
{
	...
	{
		if((notify->notify_type == AIN_OBJ_FIRED) || (notify->notify_type == AIN_HIT_BY_WEAPON) || (notify->notify_type == AIN_BUMPED_OBJ)) 
		{
...
bool Sniper::DoNotify(int me_handle, tOSIRISEventInfo *data)
{
	return true;
}
	...
	switch(mode)
	{
		case SNIPER_SNIPE:
		{
			...
			if((rand()%100) > 50)
			{
				...
				AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 0, 1.0f, -1, GF_SPEED_ATTACK | GF_KEEP_AT_COMPLETION | GF_OBJ_IS_TARGET, NULL, GST_NEG_FVEC);
		...
		case SNIPER_BASE:
		{
			...
			AI_Value(me, VF_SET, AIV_F_MAX_SPEED, &memory->base_speed);
			AI_Value(me, VF_SET, AIV_F_MAX_DELTA_SPEED, &memory->base_acc);
			memory->time_till_next_mode = 5.0f * (float)rand()/(float)RAND_MAX; + 6.0f;
		}
		break;
	}
...
bool SniperNoRun::DoNotify(int me_handle, tOSIRISEventInfo *data)
{
	return true;
}
...
void SniperNoRun::SetMode(int me, char mode)
{
	switch(mode)
	{
		...
		case SNIPER_BASE:
		{
			...
			AI_Value(me, VF_SET, AIV_F_MAX_SPEED, &memory->base_speed);
			AI_Value(me, VF_SET, AIV_F_MAX_DELTA_SPEED, &memory->base_acc);
			memory->time_till_next_mode = 5.0f * (float)rand()/(float)RAND_MAX; + 4.0f;
		}
		break;
	}
...
bool EvaderModA::DoNotify(int me_handle, tOSIRISEventInfo *data)
{
	return true;
}
...
void EvaderModA::SetMode(int me, char mode)
{
	switch(mode)
	{
		case EMA_GET_BEHIND:
		{
			...
			AI_AddGoal(me, AIG_MOVE_RELATIVE_OBJ_VEC, 0, 1.0f, -1, GF_SPEED_ATTACK | GF_KEEP_AT_COMPLETION | GF_OBJ_IS_TARGET, NULL, vec);
		}
		break;
		case EMA_NORMAL:
		{
			...
			memory->time_till_next_mode = 4.0f * (float)rand()/(float)RAND_MAX; + 6.0f;
		}
		break;
	}
...
bool FlameRAS::DoNotify(int me_handle, tOSIRISEventInfo *data)
{
	return true;
}
