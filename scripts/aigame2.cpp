...
const char *Script_names[NUM_IDS] = {
	"Samir's Pest",
	"StormTrooperBlack",
	"Creeper",
	"LukeTurret",
...
short aiObjScript::CallEvent(int event,tOSIRISEventInfo *data)
{
	...
	case EVT_AI_FRAME:
		...
		break;
	case EVT_AI_NOTIFY:
		if (!OnNotify(data->me_handle, &data->evt_ai_notify)) return 0;
		break;
	case EVT_DESTROY:
...
void aiSamirPest::OnCollide(int me_handle, int it_handle)
{
	...
	// go to hive location!
		...
		AI_AddGoal(me_handle, AIG_GET_TO_POS, 1, 1.0, -1, GF_KEEP_AT_COMPLETION | GF_NOTIFIES, 
				&pos, 
				room_index);
...
bool aiSamirPest::OnNotify(int me_handle, tOSIRISEVTAINOTIFY *data)
{
	return true;
}
...
// if we don't have a target yet, make this player a target.
	if (np && (memory->target_handle == OBJECT_HANDLE_NONE || (current_player_dist > closest_player_dist && memory->target_handle != closest_player))) {
...
// laser gunsight 
	...
	// Determine real start pos - room
		...
		int fvi_flags = FQ_CHECK_OBJS | FQ_IGNORE_POWERUPS | FQ_IGNORE_WEAPONS | FQ_IGNORE_MOVING_OBJECTS | FQ_IGNORE_NON_LIGHTMAP_OBJECTS;
...
//@@			}
//@@			memory->timer = Game_GetTime() + 2.0f + (rand() % 6)*0.5f;				// use timer to switch states.
//@@		}
		break;
...
retry_set_state:
	...
	// find a snipe point that's different from current one.
		...
		if (memory->snipe_points) {
			...
			// bot is either fresh or has been wandering, looking for a place to snipe
				...
				//	 hmm, the bot is not in a good place.  have him wander until he finds a snipe point.
					...
					Obj_Value(me_handle, VF_GET, OBJV_V_POS, &pos);
					Obj_Value(me_handle, VF_GET, OBJV_I_ROOMNUM, &room_number);
					AI_AddGoal(me_handle, AIG_WANDER_AROUND, 1, 1.0f, -1, GF_SPEED_FLEE | GF_NOTIFIES | GF_KEEP_AT_COMPLETION, &pos, room_number);
		...
		// bot will track you with no snipe points..
			...
			AI_AddGoal(me_handle, AIG_MOVE_RELATIVE_OBJ, 1, 1.0f, -1,  
							GF_OBJ_IS_TARGET | 
							GF_KEEP_AT_COMPLETION, OBJECT_HANDLE_NONE);
...
//		AI_AddGoal(me_handle, AIG_WANDER_AROUND, 3, 1.0f, -1, GF_ORIENT_VELOCITY | GF_NOTIFIES | GF_KEEP_AT_COMPLETION, &pos, room_number);
