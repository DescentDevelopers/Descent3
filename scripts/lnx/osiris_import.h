...
typedef int( *AI_FindObjOfType_fp)(int objhandle, int type, int id, bool f_ignore_init_room, int parent_handle = OBJECT_HANDLE_NONE);
...
typedef int( *Obj_Create_fp)(ubyte type,ushort id,int roomnum,vector *pos,const matrix *orient = NULL,int parent_handle = OBJECT_HANDLE_NONE,vector *initial_velocity=NULL);
...
typedef void(*Obj_SetCustomAnim_fp)(int handle, float start, float end, float time, char flags, int sound_handle = -1, char next_anim_type = -1);
...
typedef int (*AI_GetNearbyObjs_fp)(vector *pos, int init_roomnum, float rad, int *object_handle_list, int max_elements, bool f_lightmap_only, bool f_only_players_and_ais = true, bool f_include_non_collide_objects = false, bool f_stop_at_closed_doors = true);
...
typedef void (*Game_CreateRandomSparks_fp)(int num_sparks,vector *pos,int roomnum,int which_index = -1,float force_scalar = 1.0f);
...
typedef bool (*Path_GetInformation_fp)(int pathid,int point,vector *pos=NULL,int *room=NULL,matrix *orient=NULL);
