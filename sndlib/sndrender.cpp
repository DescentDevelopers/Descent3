...
//	use D3 geo
	while(!quit)
	{
		...
		if (KEY_STATE(KEY_LEFT)) {	listener_ang.h = -4096.0f*frame_time; }
		if (KEY_STATE(KEY_RIGHT)) { listener_ang.h = 4096.0f*frame_time; }
		if (KEY_STATE(KEY_UP)) { listener_ang.p = -4096.0f*frame_time; }
		if (KEY_STATE(KEY_DOWN)) {	listener_ang.p = 4096.0f*frame_time; }
		if (KEY_STATE(KEY_A)) {listener_vel.z = 8.0f*frame_time; }
		if (KEY_STATE(KEY_Z)) {listener_vel.z = -8.0f*frame_time; }
