...
//       true : success
//       false : error
bool LnxDraw_LockSurface(LnxWindow *wnd,unsigned x1,unsigned y1,unsigned x2,unsigned y2,unsigned char **ptr,int *pitch)
{
  return -1;
	if(!wnd || !ptr || !pitch)
		return false;
...
try_no_shared:
	...
	if(wnd->bHaveSharedMemory)
	{
		wnd->lpImage = XShmCreateImage(lpDisplay,wnd->lpXvisual,wnd->viVisualInfo.depth,ZPixmap,0,&wnd->shmInfo,wnd->dwWidth,h);
	...
	{		
		wnd->lpImage = XCreateImage(lpDisplay,wnd->lpXvisual,wnd->viVisualInfo.depth,ZPixmap,0,(char *)wnd->lock_ptr,wnd->dwWidth,h,16,0);
		wnd->lpImage->bitmap_bit_order = LSBFirst;
		wnd->lpImage->byte_order = LSBFirst;
		wnd->lpImage->bits_per_pixel = 16;
		wnd->lpImage->bytes_per_line = wnd->dwWidth<<1;
...
*/
    ...
    *ptr = ((unsigned char *) wnd->surface->pixels) +
            ((wnd->surface->format->BytesPerPixel * wnd->dwWidth) * linesDown);
	return true;
}
...
/*
	...
	if(wnd->bHaveSharedMemory)
	{
		if(!XShmPutImage(lpDisplay,wnd->wWindow,wnd->m_GC,wnd->lpImage,0, 0, 0, 0,wnd->dwWidth,wnd->dwHeight,True))
		{
			return -2;
		}
  	}else 
...
/*
			...
			if(!rc)
			{
				...
				}else
				{
					if(getuid()==shminfo.shm_perm.cuid)
					{
						...
						{
							//fprintf(stderr,"Shared Memory: Unable to kill stale memory\n");
							return -2;
						}
						...
						if(id==-1)
						{
							//fprintf(stderr,"Shared Memory: Unable to create shared memory block (size=%d)\n",size);
							return -2;
						}
					...
					{
						//fprintf(stderr,"Shared Memory: Using User %d's shared memory\n",shminfo.shm_cpid);
						break;
					}else
					{
						//fprintf(stderr,"Shared Memory: Stale memory (User %d, Key=0x%x) is too small\n",shminfo.shm_cpid,key);
						key++;
					}
				}
			}else
	...
	if(num_try == 0)
	{
		//fprintf(stderr,"Shared Memory: Unable to get shared memory (too many stale shared memory segments)\n");
	}
