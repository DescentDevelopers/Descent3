...
void MVE_ioCallbacks(unsigned (*fn_read)(int handle, void *buf,unsigned count))
{
	io_read = fn_read;
}
...
static bool ioReset(int h)
{
    ...
    if (strcmp(hdr->FileType, MVE_FILE_TYPE) != 0 ||
		hdr->id != ~hdr->version + 0x1234 ||
		// The following two checks may eventually be weakened.
		hdr->version != MVE_FILE_VERSION ||
		hdr->HdrSize != sizeof(mve_hdr))
		return FALSE;
...
static bool sndConfigure(unsigned rate, unsigned buflen, unsigned stereo,
			 unsigned frequency, unsigned bits16, unsigned comp16)
{
#if SOUND_SUPPORT
...
#define set_target(t) (target = (snd_queue[snd_empty].ptr+(t)+snd_buffer_caps.dwBufferBytes)% snd_buffer_caps.dwBufferBytes)
#define target_pending() ( (play_cursor<=write_cursor)?(play_cursor <= target && target < write_cursor):(play_cursor <= target || target < write_cursor))
	...
	else
	{
		...
		set_target(snd_pad);
		{
			...
			if (amt < 0) amt += snd_buffer_caps.dwBufferBytes;
	...
	{
		if (snd_empty==snd_queue_max-1) snd_empty=0;
		else ++snd_empty;
	}
#endif
	...
	if (opt_fastmode) nf_height >>= 1;
	{
		if (hicolor)
		{
			...
			for (i=0, r=0, b=0; r<32; ++r)
				for (g=0; g<32; g+=8, ++i)
					nf_trans16_hi[i+128] = nf_trans16_hi[i] = (((pal_rshift>0?r<<pal_rshift:r>>-pal_rshift)&pal_rmask) |
										((pal_gshift>0?g<<pal_gshift:g>>-pal_gshift)&pal_gmask) |
										((pal_bshift>0?b<<pal_bshift:b>>-pal_bshift)&pal_bmask) );
		}
...
static bool nfLock(void)
{
	...
	if (!nf_buf_cur || !nf_buf_prv) return FALSE;
	return TRUE;
}
...
//
void nfHiColorDecompChg(unsigned short *chgs,unsigned short *parms,unsigned char *comp,
...
static void (*sf_ShowFrame)(unsigned char *buf, unsigned bufw, unsigned bufh,unsigned sx, unsigned sy, unsigned w, unsigned h,
							unsigned dstx, unsigned dsty, unsigned nf_hicolor) = NULL;
...
void mve_ShowFrameField(unsigned char *buf, unsigned bufw, unsigned bufh,
...
void mve_ShowFrameFieldHi(unsigned char *buf, unsigned bufw, unsigned bufh,
						  unsigned sx, unsigned sy, unsigned w, unsigned h,
						  unsigned dstx, unsigned dsty, unsigned field);
	...
	if (dx<0)
		...
		if (dy<0)
			...
			if (field)
				...
				{
					(*sf_ShowFrame)(nf_dds_cur, nf_width, nf_height,0, nf_new_y, scaled_width, nf_new_h,
						dx, dy, nf_hicolor);
				}
				else
...
void MVE_sfCallbacks(void ( *fn_ShowFrame)(unsigned char *buf, unsigned bufw, unsigned bufh,unsigned sx, unsigned sy, unsigned w, unsigned h,
...
void mve_sfHiColorShowFrameChg(bool prvbuf,unsigned x, unsigned y, unsigned w, unsigned h,
							   unsigned short *chgs,unsigned dstx, unsigned dsty);


void mve_sfShowFrameChg(bool prvbuf,unsigned x, unsigned y, unsigned w, unsigned h,
						unsigned short *chgs,unsigned dstx, unsigned dsty);

static void sfShowFrameChg(int dx, int dy, unsigned short *chgs)
{
	logLabel("StartShowChg");
}
...
static void ForceVres2(unsigned char misc,unsigned vt, unsigned height, unsigned vrs /*vpos*/,
					   unsigned _2t4, unsigned msl,unsigned bdr_b, unsigned bdr_t)
{
}
...
void MVE_rmCallbacks(int (*fn_ctl)(void))
{
	rm_ctl = fn_ctl;
}
...
{
	if (hscale==3) opt_hscale_step = 3;
	else opt_hscale_step = 4;
}
...
int MVE_rmPrepMovie(int hFile, int dx, int dy, unsigned track)
{
	...
	if (!rm_track_bit) rm_track_bit = 1;
		...
		for (;;)
		{
			...
			case mcmd_nfConfig:
				{ 
					...
					//  code in sfShowFrame.
					{
						...
						if (nf_hicolor) scaled_width>>=1;  /*HACK*/
			...
			case mcmd_gfxMode:
				{ 
					...
					{
						short mode = arg->mode;
						if (opt_fastmode && (opt_fastmode&4)==0) mode |= 0x8000;
			...
			case mcmd_palMakeSynthPalette:
				{
					...
					palMakeSynthPalette(arg->base_r, arg->range_r, arg->range_rb,
						arg->base_g, arg->range_g, arg->range_gb);
					continue;
				}
...
char* MVE_strerror(int code)
{
	...
	if (code >= MVE_CTL_EXIT+1) code = MVE_CTL_EXIT+1;
	if (code <= MVE_ERR_LAST-1) code = MVE_ERR_LAST-1;
						...
						int (*fr_callback)(unsigned, unsigned,void *buf))
{
	...
	if (!mve_lpWin || !mem_alloc) return (MVE_frStream)NULL;
			...
			case mcmd_nfConfig:
				{
					...
					opt_fastmode = 0;
					if (hicolor ||(hdr.minor>=1 && arg->fqty!=1) ||!nfConfig(arg->wqty, arg->hqty,(hdr.minor>=1)?arg->fqty:1, hicolor))
					{
			...
			case mcmd_sfShowFrame:
				{
                    ...
                    arg->pal_start = INTEL_SHORT(arg->pal_start);
                    arg->pal_count = INTEL_SHORT(arg->pal_count);
					if (hdr.minor >= 1) field = arg->field;
					if (field)
					{
						result = MVE_ERR_BADFMT;
						goto done;
