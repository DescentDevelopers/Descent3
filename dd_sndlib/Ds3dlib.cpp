...
inline void	opti_8m_mix(unsigned char *cur_sample_8bit, const int num_write, int &samples_played, int *mixer_buffer16, const float l_volume, const float r_volume)
{
	int i;
...
inline void	opti_8s_mix(unsigned char *cur_sample_8bit, const int num_write, int &samples_played, int *mixer_buffer16, const float l_volume, const float r_volume)
{
	int i;
	int *mb = mixer_buffer16;
...
inline void	opti_16m_mix(short *cur_sample_16bit, const int num_write, int &samples_played, int *mixer_buffer16, const float l_volume, const float r_volume)
{
	int i;
	int *mb = mixer_buffer16;
...
inline void	opti_16s_mix(short *cur_sample_16bit, const int num_write, int &samples_played, int *mixer_buffer16, const float l_volume, const float r_volume)
{
	int i;
	int *mb = mixer_buffer16;
...
int GamePanToDsPan(float pan)
{
	...
	if(ds_pan > MAX_DS_PAN) ds_pan = MAX_DS_PAN;
	if(ds_pan < -MAX_DS_PAN) ds_pan = -MAX_DS_PAN;
...
inline int sb_get_status(sound_buffer_info *sb)
{
	...
	if (sb->m_mixer_type == SOUND_MIXER_AUREAL) {
		...
		if (sb->m_snd_obj) {
			...
			if (status & A3D_STATUS_PLAYING) retflags |= SB_STATUS_PLAYING;
		}
		else {
	...
	if (sb->m_mixer_type != SOUND_MIXER_SOFTWARE_16) {
		...
		if (sound_ptr) {
			...
			if (status & DSBSTATUS_PLAYING) retflags |= SB_STATUS_PLAYING;
		}
		else {
			retflags |= SB_STATUS_INVALID;
		}
...
{
	if (!ll_sound_ptr->m_in_sound_frame) ll_sound_ptr->m_pending_actions = true;
	...
	//	f_volume = f_volume * 1.2;
		if (f_volume > 1.0f) f_volume = 1.0f;
		A3D_SetSourceVolume(sb->m_snd_obj, f_volume);
//		A3D_SetMinMaxDistance(sb->m_snd_obj, Sounds[sb->m_sound_index].min_distance, Sounds[sb->m_sound_index].max_distance);
...
inline void sb_adjust_properties_2d(sound_buffer_info *sb, float f_volume, float f_pan, ushort frequency)
{
	...
	if (sb->m_mixer_type == SOUND_MIXER_AUREAL) {
		if (sb->m_snd_obj) {
			...
			if (f_pan < 0.0f) r_pan = r_pan + f_pan;
			if (f_pan > 0.0f) l_pan = l_pan - f_pan;
			A3D_SetSourceVolume(sb->m_snd_obj, f_volume);
...
// this code will assure that this function will not be called when  sound system is in error mode.
	...
	while(current_slot < ll_sound_ptr->m_sound_mixer.m_max_sounds_played)
	{
		...
		if((cur_buf->m_status != SSF_UNUSED) &&
         !(cur_buf->m_status & SSF_PAUSED))
		{
		...
		//	ASSERT(sample_16bit || sample_8bit);
			...
			if(cur_buf->m_status & (SSF_PLAY_LOOPING | SSF_PLAY_STREAMING))  // Looping sample's process is broken up into linear pieces
			{
				if(f_loop)
				{
					ASSERT(num_write >= 0);
				...
				if(num_write < num_samples) f_loop = true;
				else f_loop = false;
			...
			{
				// The number of samples to write to the primary buffer
				num_write = ((num_samples) < (np_sample_length - samples_played))?(num_samples):(np_sample_length - samples_played);
				...
				if(skip_interval == 1)
				{
					...
					for(i = 0; i < (num_write << 1); i += 2)
					{
						...
						if(sample_16bit)
						{
							...
							{
								sample = ((int)sample_16bit[samples_played ^ 0x0001] +
											 (int)sample_16bit[samples_played + 1]) >> 1;
							}
							else
					...
					for(i = 0; i < (num_write << 1); i += 2)
					{
						...
						if(sample_16bit)
						{
							...
							case 0:
								...
								break;
							case 1:
								sample = (sample_16bit[samples_played - 1]*3 +
											 sample_16bit[samples_played + 3]) >> 2;
								break;
							case 2:
								sample = (sample_16bit[samples_played - 2] +
											 sample_16bit[samples_played + 2]) >> 1;
								break;
							case 3:
								sample = (sample_16bit[samples_played - 3] +
											 sample_16bit[samples_played + 1]*3) >> 2;
								break;
							}
						}
						else
						{
							...
							case 1:
								sample = (((((int)sample_8bit[samples_played - 1]) - 128) << 8)*3 +
											 ((((int)sample_8bit[samples_played + 3]) - 128) << 8)) >> 2;
								break;
							case 2:
								sample = (((((int)sample_8bit[samples_played - 2]) - 128) << 8) +
											 ((((int)sample_8bit[samples_played + 2]) - 128) << 8)) >> 1;
								break;
							case 3:
								sample = (((((int)sample_8bit[samples_played - 3]) - 128) << 8) +
											 ((((int)sample_8bit[samples_played + 1]) - 128) << 8)*3) >> 2;
								break;
							}
						}
...
stream_done:
			...
			if(cur_buf->m_status & SSF_PLAY_STREAMING)
			{
				if(f_loop)
				{
					...
					{
						cur_buf->play_info->m_stream_data = (*cur_buf->play_info->m_stream_cback)(cur_buf->play_info->user_data, cur_buf->play_info->m_stream_handle, &cur_buf->play_info->m_stream_size);
...
//						mprintf((0, "Data %X, length %d\n", cur_buf->play_info->m_stream_data, cur_buf->play_info->m_stream_size));
			...
			if (f_loop) goto looping;

			done:

			if(cur_buf->play_info->m_samples_played >= (np_sample_length) && !(cur_buf->m_status & (SSF_PLAY_LOOPING | SSF_PLAY_STREAMING))) {
...
void StreamFill(int start_byte, int num_bytes)
{
	...
	switch(m_sb_info.m_lp_looping_buffer->Lock(start_byte, num_bytes, (void **)&ptr1, (DWORD *)&len1, (void **)&ptr2, (DWORD *)&len2, 0))
	{
		...
		case DSERR_BUFFERLOST:
			if(m_sb_info.m_lp_primary_buffer->Restore() == DS_OK && 
				m_sb_info.m_lp_looping_buffer->Restore() == DS_OK)
				goto TryLockAgainLabel;
			break;
	}
}
...
//		}
		...
		if(writep == playp)
		{
			...
			if((try_count == 1) && (m_sb_info.m_lp_primary_buffer->Restore() == DS_OK &&
				                     m_sb_info.m_lp_looping_buffer->Restore() == DS_OK))
			{
		...
		// Insert mixer code
		{
			...
			if(sb_info->LastPlayPos < sb_info->NextWritePos)
			{
				...
				{
					if(!sb_info->m_f_secondary_looping) goto done;
...
long win_llsSystem::CreateDSBuffer(int buffer_type, LPDIRECTSOUNDBUFFER *lp_lp_dsb, LPDIRECTSOUND3DBUFFER *lp_lp_dsb_3d, ulong sound_bytes, ulong frequency, bool f_is_stereo, bool f_is_16_bit) 
{
	DSBUFFERDESC dsbd;
	tWAVEFORMATEX fmt;
	...
	if (!m_f_sound_lib_init) return 0;

	if (lp_lp_dsb) { *lp_lp_dsb = NULL; }
	if (lp_lp_dsb_3d) { *lp_lp_dsb_3d = NULL; }
...
BOOL CALLBACK LLEnumCallback(LPGUID lp_guid, 
						  LPCSTR lpstr_description, 
						  LPCSTR lpstr_module, 
						  LPVOID lp_Context) 
{
...
// reset error system.
	...
	if(sos) {
		...
		if(m_f_sound_lib_init) return 1;
...
retry_mixer_init:
	...
	// can to 3d stuff too :)
		...
		f16bit = true;
		hresult = CreateDSBuffer(SBT_PRIMARY, &m_sb_info.m_lp_primary_buffer, 
							NULL, 
							0, 
							22050,				// frequency
							true,					// stereo
	...
	// start primary buffer
		f16bit = (mixer_type == SOUND_MIXER_DS_16 || mixer_type == SOUND_MIXER_DS3D_16 || mixer_type == SOUND_MIXER_CREATIVE_EAX);
		hresult = CreateDSBuffer(SBT_PRIMARY, &m_sb_info.m_lp_primary_buffer, 
								NULL, 
								0, 
								22050,			// frequency
...
//	buffered method doesn't use threads at all.
	if (m_mixer_type != SOUND_MIXER_SOFTWARE_16 && m_mixer_type != SOUND_MIXER_NONE && m_sound_mixer.m_loop_method != DSLOOP_BUFFER_METHOD) {
	...
	switch(m_mixer_type)
	{
	case SOUND_MIXER_SOFTWARE_16:	mprintf((0, "Software 16\n")); break;
	case SOUND_MIXER_DS_8: mprintf((0, "DS 8\n"));	break;
	case SOUND_MIXER_DS_16:	mprintf((0, "DS 16\n")); break;
	case SOUND_MIXER_DS3D_16: mprintf((0, "DS3D 16\n"));	break;
	case SOUND_MIXER_AUREAL: mprintf((0, "Aureal 3D\n")); break;
	case SOUND_MIXER_CREATIVE_EAX: mprintf((0, "Creative EAX\n")); break;
	case SOUND_MIXER_NONE: mprintf((0, "None\n")); break;
	default:
...
error_sub:
	...
	// Only gets here if there was an error
		...
		if(m_sound_mixer.m_sound_cache != NULL) delete[] m_sound_mixer.m_sound_cache;
		m_f_sound_lib_init = 0;
		m_mixer_type = SOUND_MIXER_NONE;
	}
	...
	for(current_slot = 0; current_slot < m_sound_mixer.m_max_sounds_played; current_slot++) 
	{
		...
		if(sb->m_status == SSF_UNUSED) 
		{
			if(!((m_mixer_type == SOUND_MIXER_DS_8 || m_mixer_type == SOUND_MIXER_DS_16 
				...
				m_mixer_type == SOUND_MIXER_CREATIVE_EAX)  && sb->s))
				return current_slot;
...
int win_llsSystem::PlaySound2d(play_information *play_info, int sound_index, float f_volume, float f_pan, bool f_looped) 
{
...
void win_llsSystem::LoopStartStreaming(sound_buffer_info *sb, int buffer_type, float volume, float pan, pos_state *cur_pos)
{
...
inline int win_llsSystem::MakeUniqueId(int sound_slot){
	return ((((int)m_total_sounds_played)<<8) + sound_slot);
}
	...
	for(current_slot = 0; current_slot < m_sound_mixer.m_max_sounds_played; current_slot++) 
	{
		...
		if(sb->m_status != SSF_UNUSED) 
		{
			...
			if (sb->m_status & SSF_PLAY_LOOPING) {
				if (sb->m_status & SSF_BUFFERED_LOOP) buf_loop_counter++;
				loop_counter++;
			}
			if (sb->m_status & SSF_PLAY_STREAMING) stream_counter++;

		#ifdef _DEBUG
			if (sb->play_info->priority == SND_PRIORITY_CRITICAL) n_p5++;
			else if (sb->play_info->priority == SND_PRIORITY_HIGHEST) n_p4++;
			else if (sb->play_info->priority == SND_PRIORITY_HIGH) n_p3++;
			else if (sb->play_info->priority == SND_PRIORITY_NORMAL) n_p2++;
			else if (sb->play_info->priority == SND_PRIORITY_LOW) n_p1++;
			else if (sb->play_info->priority == SND_PRIORITY_LOWEST) n_p0++;
		#endif
		}
	}
...
bool win_llsSystem::LockSound(int sound_uid)
{
	return false;
}

bool win_llsSystem::UnlockSound(int sound_uid)
{
	return false;
}
...
bool win_llsSystem::CheckAndForceSoundDataAlloc(int sound_index)
{
	...
	result = SoundLoadWaveFile(SoundFiles[sound_file_index].name, Sounds[sound_index].import_volume, sound_file_index, (m_sound_quality == SQT_HIGH), true);
	...
	for(i = 0; i < MAX_SOUNDS; i++)
	{
		if (Sounds[i].used != 0) 
		{
			...
			if(SoundFiles[j].sample_16bit && m_sound_quality == SQT_NORMAL) 
			{
				...
				// NOTE:  Interesting note on sound conversion:  16 bit sounds are signed (0 biase).  8 bit sounds are unsigned (+128 biase).
...
char win_llsSystem::GetSoundQuality(void)
{
	return m_sound_quality;
}
...
char win_llsSystem::GetSoundMixer(void)
{
	return m_mixer_type;
}
...
#endif
	if(IS_3D_MIXER(m_mixer_type)) {
		m_lp_listener->SetOrientation(cur_pos->orient->fvec.x, cur_pos->orient->fvec.y, cur_pos->orient->fvec.z, cur_pos->orient->uvec.x, cur_pos->orient->uvec.y, cur_pos->orient->uvec.z, DS3D_DEFERRED);
...
void win_llsSystem::AdjustSound(int sound_uid, float f_volume, float f_pan, unsigned short frequency) 
{
	...
	play_info->left_volume = play_info->right_volume = f_volume;
	if (f_pan < 0.0) play_info->right_volume += f_volume * f_pan;
	else play_info->left_volume -= f_volume * f_pan;
...
//	sound_buffer_info *sb = &m_sound_mixer.m_sound_cache[current_slot];
	...
	else 
	{
		...
		{
			volume *= (1.0 - ((dist - Sounds[m_sound_mixer.m_sound_cache[current_slot].m_sound_index].min_distance) / (Sounds[m_sound_mixer.m_sound_cache[current_slot].m_sound_index].max_distance - Sounds[m_sound_mixer.m_sound_cache[current_slot].m_sound_index].min_distance)));
		}
...
int win_llsSystem::PlaySound3d(play_information *play_info, int sound_index, pos_state *cur_pos, float adjusted_volume, bool f_looped, float reverb)
{
	short sound_slot;
	DWORD ds_flags = 0;
	float volume;
	...
	if(Sounds[sound_index].used == 0) return -1;
	...
	if(!IS_3D_MIXER(m_mixer_type))
	{
		...
		{
			volume *= (1.0 - ((dist - Sounds[sound_index].min_distance) / (Sounds[sound_index].max_distance - Sounds[sound_index].min_distance)));
		}
...
//		mprintf((0, "SL Play sound on slot %d, TP = %d UI = %X\n", sound_slot, TotalSoundsPlayed, m_sound_mixer.m_sound_cache[sound_slot].m_unique_id));
	}
...
int win_llsSystem::SetSoundPos(int sound_uid, int pos) 
{
	...
	if (!m_f_sound_lib_init) return -1;
	if((current_slot = ValidateUniqueId(sound_uid)) == -1) return -1;
	if(m_sound_mixer.m_sound_cache[current_slot].m_status == SSF_UNUSED) return -1;
...
#ifdef SUPPORT_AUREAL
	case SOUND_MIXER_AUREAL:
		sb->m_snd_obj = A3D_CreateSSource(sound_length,
									f_sample_16bit ? 16 : 8, 
									f_is_stereo, 
									(dynamic) ? true : false,
									22050,
									(buftype == SBT_3D) ? true : false);
		if (!sb->m_snd_obj) {
			Int3();
			sb->m_status = SSF_UNUSED;
			return false;
...
#endif
	default:
		CreateDSBuffer(buftype, &sb->m_sound_buffer, 
	   		&sb->m_sound_buffer_3d, 
				sound_length, 
				22050, 
				f_is_stereo, 
				f_sample_16bit);  
...
bool win_llsSystem::PlaySoundBuffer(sound_buffer_info *sb, tPSBInfo *psb)
{
	...
	if (!m_f_sound_lib_init) return false;
	if (!m_in_sound_frame) m_pending_actions = true;
...
TryPlayAgainLabel:
	if (psb) {
		...
		if (f_looping && m_sound_mixer.m_loop_method == DSLOOP_BUFFER_METHOD && sb->s->loop_step != 0 && (sb->m_status&SSF_BUFFERED_LOOP)) {
			f_looping = false;
		}
	}
	...
	case SOUND_MIXER_AUREAL:
		{
			...
			case SND_PRIORITY_HIGHEST: priority = 1.0f; break;
			case SND_PRIORITY_HIGH: priority = 0.85f; break;
			case SND_PRIORITY_NORMAL: priority = 0.60f; break;
			case SND_PRIORITY_LOW: priority = 0.30f; break;
			case SND_PRIORITY_LOWEST: priority = 0.1f; break;
			default: priority = 0.5f;
			}
