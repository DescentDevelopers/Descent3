...
void sb_stream_copy(sound_buffer_info *sb, char *ptr, DWORD len)
{
	...
	if (len)
	{
		if(sb->play_info->m_stream_cback && sb->s->current_position)
		{
			...
			sb->s->current_position = (char *) (*sb->play_info->m_stream_cback)(sb->play_info->user_data, sb->play_info->m_stream_handle, &new_len);
...
/*		sb->s->hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
		if (sb->m_mixer_type == SOUND_MIXER_AUREAL) {
			...
			Int3();												// unimplemented for normal DirectSound
			CloseHandle(sb->s->hEvent);
			sb->s->hEvent = NULL;
		}
*/
...
{
	if (!sb->s) return;
	if (!(sb->m_status & SSF_BUFFERED_LOOP)) return;
