		...
		switch(ckid)
		{
			case 0x20746D66: //Format Chunk
				...
				switch(fmttag)
				{
					...
					default:
					case 0x0000: strcpy(format_type, "WAVE_FORMAT_UNKNOWN"); break;
					case 0x0001: strcpy(format_type, "WAVE_FORMAT_PCM"); break;
					case 0x0002: strcpy(format_type, "WAVE_FORMAT_ADPCM"); break;
					case 0x0005: strcpy(format_type, "WAVE_FORMAT_IBM_CVSD"); break;
					case 0x0006: strcpy(format_type, "WAVE_FORMAT_ALAW"); break;
					case 0x0007: strcpy(format_type, "WAVE_FORMAT_MULAW"); break;
					case 0x0010: strcpy(format_type, "WAVE_FORMAT_OKI_ADPCM"); break;
					case 0x0011: strcpy(format_type, "WAVE_FORMAT_DVI_ADPCM"); break;
					case 0x0015: strcpy(format_type, "WAVE_FORMAT_DIGISTD"); break;
					case 0x0016: strcpy(format_type, "WAVE_FORMAT_DIGIFIX"); break;
					case 0x0020: strcpy(format_type, "WAVE_FORMAT_YAMAHA_ADPCM"); break;
					case 0x0021: strcpy(format_type, "WAVE_FORMAT_SONARC"); break;
					case 0x0022: strcpy(format_type, "WAVE_FORMAT_DSPGROUP_TRUESPEECH"); break;
					case 0x0023: strcpy(format_type, "WAVE_FORMAT_ECHOSC1"); break;
					case 0x0024: strcpy(format_type, "WAVE_FORMAT_AUDIOFILE_AF18"); break;
					case 0x0101: strcpy(format_type, "IBM_FORMAT_MULAW"); break;
					case 0x0102: strcpy(format_type, "IBM_FORMAT_ALAW"); break;
					case 0x0103: strcpy(format_type, "IBM_FORMAT_ADPCM"); break;
					case 0x0200: strcpy(format_type, "WAVE_FORMAT_CREATIVE_ADPCM"); break;
				}
...
//					cf_ReadBytes ((ubyte *)SoundFiles[sound_file_index].sample_8bit,cksize,cfptr);
					...
					if(aligned_size != cksize) memset(SoundFiles[sound_file_index].sample_8bit + cksize, 128, num_needed);
#endif
				} 
				else 
				{
...
//					SoundFiles[sound_file_index].sample_16bit = (short *) mem_malloc(2*cksize);
...
//						SoundFiles[sound_file_index].sample_16bit[i++] = tmp_snd;
						SoundFiles[sound_file_index].sample_16bit[i++] = INTEL_SHORT(SoundFiles[sound_file_index].sample_16bit[count]);
					}
#else
					...
					{
						num_needed = (SOUND_FILE_SAMPLE_ALIGNMENT - ((SoundFiles[sound_file_index].sample_length - 1) % SOUND_FILE_SAMPLE_ALIGNMENT)) * 2;
					...
					{
						SoundFiles[sound_file_index].sample_16bit[count] = INTEL_SHORT(SoundFiles[sound_file_index].sample_16bit[count]);
					}
	...
	else if(SoundFiles[sound_file_index].sample_8bit != NULL && !f_high_quality) 
	{
		...
		{
			SoundFiles[sound_file_index].sample_8bit[count] = ((((int)SoundFiles[sound_file_index].sample_8bit[count]) - 127)*CLIP_ATTENUATION*percent_volume) + 127;
		}
	}
	...
	else if(SoundFiles[sound_file_index].sample_8bit == NULL && !f_high_quality) 
	{
		...
		SoundFiles[sound_file_index].sample_8bit = (unsigned char *)GlobalAlloc(0,SoundFiles[sound_file_index].sample_length);
		GlobalLock(SoundFiles[sound_file_index].sample_8bit);
		...
		for(count = 0; count < (int)SoundFiles[sound_file_index].sample_length; count++) 
		{
			SoundFiles[sound_file_index].sample_8bit[count] = (unsigned char)((((int)SoundFiles[sound_file_index].sample_16bit[count]) + 32767) >> 8);
		}
	...
	{
		SoundFiles[sound_file_index].sample_16bit =(short *)GlobalAlloc(0, SoundFiles[sound_file_index].sample_length*sizeof(short));
		GlobalLock(SoundFiles[sound_file_index].sample_16bit);

		// NOTE:  Interesting note on sound conversion:  16 bit sounds are signed (0 biase).  8 bit sounds are unsigned (+128 biase).
		for(count = 0; count < (int)SoundFiles[sound_file_index].sample_length; count++) 
		{
			SoundFiles[sound_file_index].sample_16bit[count] = (((short)SoundFiles[sound_file_index].sample_8bit[count]) - 128) * 256;
		}
