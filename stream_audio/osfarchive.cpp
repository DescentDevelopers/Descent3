...
//	read in rest of header.
	...
	if (m_type == OSF_DIGITAL_STRM) {
		if (m_rate == 11) m_rate = 11025;
		else if (m_rate == 22) m_rate = 22050;
		else if (m_rate == 44) m_rate = 44100;
...
bool OSFArchive::WriteBlock(ubyte *blk, int size)
{
	return (cf_WriteBytes(blk, size, m_fp) < size) ? false : true;
}
