...
ddgr_error 	ddgr_os_surf_InitVideo(ddgr_surface *sf)
{
	...
	try
	{
		...
		if (userCanSelect)
		{
			...
			if (macErr)
				throw (CMacOSSurfErr("Error while allowing user to choose a monitor.",
									macErr, DDGRERR_DRIVERINIT));
		}
		else
		{
			...
			macErr = DSpFindBestContext( &(gFullScreenData.m_screen_attr),
										 &(gFullScreenData.m_context) );
			if (macErr)
				throw (CMacOSSurfErr("Error while autoselecting a monitor.",
									macErr, DDGRERR_DRIVERINIT));
		}
...
//	mprintf((0, "Clearing OS Surface!\n"));
	...
	try
	{
		if (!(sf->flags & SURFFLAG_BACKBUFFER))
		{
			...
			macErr = DSpContext_GetBackBuffer(gFullScreenData.m_context,
											  kDSpBufferKind_Normal,
											  &macBackBufferPtr);
			if (macErr || !macBackBufferPtr)
			{
				throw (CMacOSSurfErr("Could not obtain full screen surface backbuffer ptr!",
									 macErr,
									 DDGRERR_FAIL));
			}
			...
			if (!dstPixMapH)
			{
				throw (CMacOSSurfErr("Could not obtain full screen pixmaps for clearing os surface",
										 macErr,
										 DDGRERR_FAIL));
			}
			...
			if (!lockSuccess)
			{
				throw (CMacOSSurfErr("Error: Could not lock destination pixels for flipping!",
									 0,
									 DDGRERR_FAIL));
			}
...
ddgr_error 	ddgr_os_surf_Create(ddgr_surface *sf)
{
	...
	try
	{
		...
		if (surfDataObj == nil)
		{
			throw (CMacOSSurfErr("Error allocating memory for mac os surface data object!",
								 noErr,
								 DDGRERR_OUTOFMEMORY)); 
		}
		...
		mprintf((0,"OS SURFACE CREATE: surface bounds rect = %d, %d, %d, %d (t,l,b,r).  Is this correct?\n", 
				surfBoundsRect.top,
				surfBoundsRect.left,
				surfBoundsRect.right,
				surfBoundsRect.bottom));
		
		macErr = NewGWorld(&newOffscreen, sf->bpp, &surfBoundsRect, nil, nil, 0);
		if (macErr)
		{
			throw (CMacOSSurfErr("Error allocating memory for mac offscreen buffer!",
								 noErr,
								 DDGRERR_OUTOFMEMORY)); 
		}
...
void ddgr_os_surf_Destroy(ddgr_surface *sf)
{
	...
	try
	{
		...
		if (surfDataObj == nil)
		{
			throw (CMacOSSurfErr("Error:  No data object associated with offscreen surface!",
								 0,
								 DDGRERR_FAIL));
		}
...
//	mprintf((0, "Clearing OS Surface!\n"));
	...
	try
	{
		...
		if (!lockSuccess)
		{
			throw (CMacOSSurfErr("Error: Could not lock destination pixels for blit!",
								 0,
								 DDGRERR_FAIL));
		}
		...
		ASSERT((bitsPerPixel == BPP_DEFAULT) ||
			   (bitsPerPixel == BPP_8)  ||
			   (bitsPerPixel == BPP_16) ||
			   (bitsPerPixel == BPP_24) ||
			   (bitsPerPixel == BPP_32));
...
ddgr_error	ddgr_os_surf_Blt(ddgr_surface *dsf, int dx, int dy,
				ddgr_surface *ssf, int sx, int sy, int sw, int sh)
{
	...
	try
	{
		...
		{
			throw (CMacOSSurfErr("Could not obtain full screen pixmaps for blitting os surface",
									 macErr,
									 DDGRERR_FAIL));
		}
		...
		if (!lockSuccess)
		{
			throw (CMacOSSurfErr("Error: Could not lock source pixels for blit!",
								 0,
								 DDGRERR_FAIL));
		}
...
ddgr_error	ddgr_os_surf_Lock(ddgr_surface *sf)
{
	...
	try
	{
		...
		{
			throw (CMacOSSurfErr("Attempted to lock surface with invalid surftype!",
								 0,
								 DDGRERR_FAIL));
		}
...
float
ddgr_os_surf_GetAspectRatio(void)
{
	ASSERT(!Mac_DDGR_Lib.IsWindowed());
	...
	aspect = (float) gFullScreenData.m_screen_attr.displayWidth /
			 (float) gFullScreenData.m_screen_attr.displayHeight;
