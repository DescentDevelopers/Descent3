...
CAISettingsDlg::CAISettingsDlg(t_ai_info *ai_info, CWnd *pParent /*=NULL*/) : CDialog(CAISettingsDlg::IDD, pParent) {
...
BOOL CAISettingsDlg::OnInitDialog() {
  ...
  for (i = 0; i < MAX_AI_INIT_MOVEMENT_TYPES; i++) {
    if (!stricmp("", Ai_movement_type_strings[i]))
      break;
  ...
  if (!stricmp(m_ai_movement_type, "FLYING")) {
    for (i = 0; i < MAX_AI_INIT_MOVEMENT_SUBTYPES; i++) {
      if (!stricmp("", Ai_movement_subtype_flying_strings[i]))
        break;
      SendDlgItemMessage(IDC_AI_MOVEMENT_SUBTYPE_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Ai_movement_subtype_flying_strings[i]);
    }
    ...
    for (i = 0; i < MAX_AI_INIT_MOVEMENT_SUBTYPES; i++) {
      if (!stricmp("", Ai_movement_subtype_walking_strings[i]))
        break;
      SendDlgItemMessage(IDC_AI_MOVEMENT_SUBTYPE_COMBO, CB_ADDSTRING, 0,
                         (LPARAM)(LPCTSTR)Ai_movement_subtype_walking_strings[i]);
    }
    SendDlgItemMessage(IDC_AI_MOVEMENT_SUBTYPE_COMBO, CB_SELECTSTRING, 0, (LPARAM)(LPCTSTR)m_ai_movement_subtype);
  } else {
...
void CAISettingsDlg::OnSelchangeAiMovementTypeCombo() {
  ...
  if (!stricmp(m_ai_movement_type, "FLYING")) {
    ...
    SendDlgItemMessage(IDC_AI_MOVEMENT_SUBTYPE_COMBO, CB_SELECTSTRING, 0,
                       (LPARAM)(LPCTSTR)Ai_movement_subtype_flying_strings[0]);
  } else if (!stricmp(m_ai_movement_type, "WALKING")) {
    for (i = 0; i < MAX_AI_INIT_MOVEMENT_SUBTYPES; i++) {
      if (!stricmp("", Ai_movement_subtype_walking_strings[i]))
        break;
    ...
    SendDlgItemMessage(IDC_AI_MOVEMENT_SUBTYPE_COMBO, CB_SELECTSTRING, 0,
                       (LPARAM)(LPCTSTR)Ai_movement_subtype_walking_strings[0]);
  } else {
    SendDlgItemMessage(IDC_AI_MOVEMENT_SUBTYPE_COMBO, CB_SELECTSTRING, 0, (LPARAM)(LPCTSTR) "");
  }
}
